PENDING
- cooking ถ้าให้จำ user state เป็น session โดยไม่ต้อง login ทำได้มะ จุดประสงค์อยากให้คนเข้าหน้านี้ได้เลยโดยไม่ต้อง login ใช้ได้เลย ข้อจำกัดคืออะไร
- cooking เวลา check ของให้สีคนละอันกัน ระหว่าง มี กับ ไม่มี
- เพิ่ม แก้ไข menu ยังไง, ทำให้เพิ่มรูปให้ด้วย
- ถ้าส่งคำขอให้ หารูปจากเน็ตให้ได้ยิ่งดีถ้าทำได้
- เอาอันอื่น กับ สุ่มให้เลย ต่างกันยังไง
❌ จังหวัด "กรุงเทพ" ไม่ถูกต้อง กรุณาลองใหม่- เพิ่ม ลด ingredient ยังไง, เครื่องปรุงอยู่ไหน
- ขอรายการอาหาร เพื่อไปหารูป
- อยากดูเมนูทั้งหมด และมี animation ระหว่างสุ่มหรือ เอาอันอื่น เพิ่มความน่าสนใจแบบ slot machine

- อยากได้หน้า dashboard ของ http://localhost:3000/bot/ ตอนนี้มันวิ่งไป http://localhost:3000/bot/platforms ดูแปลกๆ ไม่มี landing ทำแบบไหนดี
