...
Index Discord threads in forum channels
  ...
  → Hybrid search: Postgres ILIKE + Meilisearch
  → Fallback to Postgres if Meilisearch unavailable
```
...
- Auto-fallback to Postgres ILIKE if not running
...
hybridSearch(keyword, guildId, channelId, sort)
  → Promise.all([Postgres ILIKE post_name, Meilisearch full-text])
  → Merge + dedupe by post_id
  → Filter null post_name
  → Sort: both-match first, then newest
  → Fallback to Postgres-only if Meilisearch unavailable
```
...
- pg driver **auto-parse JSON/JSONB column เป็น JS object** ไม่ใช่ string
...
1. ตั้งห้องข่าวสาร — **ตั้งรายกลุ่มที่ /org/settings/social** (การ์ด "การตั้งค่ารายกลุ่ม") ตั้งแต่ 2026-08-12
   ...
   · ลำดับตัดสินเต็มๆ อ่านจากโค้ดตรงๆ: `getNewsChannelId(guildId, groupName)` ใน [`services/newsShare.js`](../../../services/newsShare.js)
     (เดิมชี้ไป `md/rules/DESIGN.md §ห้องข่าวสาร` ซึ่ง**ไม่เคยมีหัวข้อนั้นอยู่จริง** — แก้ 2026-09-19)
