...
**Phase 2 — ownership migration ต่อ feature (incremental):**
- เพิ่ม `user_id` + `org_id` ใน core tables + backfill (user_id จาก discord_id ผ่าน dc_members · org_id จาก guild_id ผ่าน dc_guilds.org_id)
- ทำ **docs cluster ก่อน** (4 ตาราง: docs_activity_entries [member+payer = 2 user_id col], docs_payers, docs_projects, docs_signatures) → แล้ว calling_starred, case_assignees, dc_user_config, dc_user_identities
