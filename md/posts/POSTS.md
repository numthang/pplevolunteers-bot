...
> ⚠️ **2026-08-09 — เอา AI auto-split ออก, 2026-08-12 — เอากลับมาเป็นตัวเลือกที่ user กดเอง**
...
> 2026-08-12: user อยากได้ series กลับมา แต่ต้อง **user เป็นคนกดเลือกเอง** ไม่ใช่ AI ตัดสินใจแบบเดิม — กล่องไอเดียมีตัวเลือก **[1 ตอน | Series]**:
> - **1 ตอน (default)** — เหมือนเดิมทุกอย่าง: ยิง `ai/compose` ไม่ส่ง `episodeCount` → ได้ 1 โพสต์ → เด้งเข้า `/posts/[id]`
> - **Series** — user พิมพ์จำนวนตอนเอง (2-12) → ยิง `ai/compose` พร้อม `episodeCount` → AI ร่าง body เต็มทุกตอนทันที (ไม่ใช่แค่โครง/gist แบบ outline เดิม) คืน `{category, seriesName, posts:[]}` → สร้าง `post_episodes` ครบทุกแถวพร้อมกัน หมวดเดียวกัน, title = `<seriesName> EP.xx: <ชื่อตอน>` (เลข EP. ต่อฝั่ง server กันเลขเพี้ยน) → ไม่มีหน้าเดียวให้พาไป จึงกลับมาที่ `/posts` กรองตามหมวดที่ได้ พร้อมข้อความแจ้งสร้างสำเร็จ
