...
> ⚠️ **2026-08-09 — เอา AI auto-split ออก, 2026-08-12 — เอากลับมาเป็นตัวเลือกที่ user กดเอง (2 รอบ)**
> 2026-08-09: user โยนบทความยาวเข้าไปแล้วได้ร่าง 4–5 อันแตกออกมาโดย AI ตัดสินใจเอง ซึ่ง "เข้าใจไม่ตรงกัน" ตั้งแต่แรก — ตัด `/api/posts/ai/outline` ทิ้ง เหลือแต่ `/api/posts/ai/compose` (โพสต์เดียวเสมอ)
> 2026-08-12 (รอบแรก): user อยากได้ series กลับมา แต่ต้อง **user เป็นคนกดเลือกเอง** ไม่ใช่ AI ตัดสินใจแบบเดิม — เพิ่มตัวเลือก **[1 ตอน | Series]** พร้อมช่องกรอกจำนวนตอน (2-12)
> 2026-08-12 (รอบสอง — ค่าปัจจุบัน): user ถอยเรื่องช่องกรอกจำนวนตอนออก — **AI กลับมาตัดสินใจจำนวนตอนเอง (1-12) จากเนื้อหา** เหมือนเดิม แต่ต่างจาก 2026-08-09 ตรงที่ยังต้อง **user กดปุ่ม "Series" เลือกโหมดเองก่อน** (ไม่ใช่ AI auto-split แบบไม่ถามใครแล้วเงียบๆ) — user มักพิมพ์ระบุจำนวนตอนไว้ในเนื้อหาเอง (เช่น "แบ่งเป็น 5 ตอน") prompt จึงสั่งให้ AI อ่านจากตรงนั้นก่อน ไม่ระบุมาค่อยให้ AI ตัดสินใจเอง
> กล่องไอเดียมีตัวเลือก **[1 ตอน | Series]**:
> - **1 ตอน (default)** — เหมือนเดิมทุกอย่าง: ยิง `ai/compose` ไม่ส่ง `series` → ได้ 1 โพสต์ → เด้งเข้า `/posts/[id]`
> - **Series** — ยิง `ai/compose` พร้อม `series: true` → AI ตัดสินใจจำนวนตอนเอง (1-12) แล้วร่าง body เต็มทุกตอนทันที (ไม่ใช่แค่โครง/gist แบบ outline เดิม) คืน `{category, seriesName, posts:[]}` → สร้าง `post_episodes` ครบทุกแถวพร้อมกัน หมวดเดียวกัน, title = `<seriesName> EP.xx: <ชื่อตอน>` (เลข EP. ต่อฝั่ง server กันเลขเพี้ยน) → ไม่มีหน้าเดียวให้พาไป จึงกลับมาที่ `/posts` กรองตามหมวดที่ได้ พร้อมข้อความแจ้งสร้างสำเร็จ
> ยังไม่มีตาราง `post_series`/`seq` กลับมา — จัดกลุ่มด้วย `category` เหมือนเดิมทุกประการ, `EP.xx` เป็นแค่ข้อความในชื่อโพสต์
...
ข้อความอยู่กลางการ์ด · **สัดส่วนเลือกเองเหมือนตอนครอปรูป** (2026-08-11 — เดิมตรึง 4:5 อย่างเดียว):

| คีย์ | ขนาด canvas |
|---|---|
| `1:1` | 1080×1080 |
| `4:5` (ตั้งต้น) | 1080×1350 |
| `16:9` | 1920×1080 |

คีย์อยู่ที่ `PLAIN_SIZES_BY_ASPECT` (lib/quoteStyles.js) **ต้องตรงกับ `ASPECTS`** ในโมดัล ·
ค่ามั่วตกเป็น 4:5 · ทุกอย่างในการ์ดคิดจากความกว้าง canvas จึงสเกลตามสัดส่วนเองหมด
