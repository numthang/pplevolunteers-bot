...
`open` (รับเรื่องแล้ว) → `in_progress` (กำลังดำเนินการ) → `resolved` (แก้ไขแล้ว) / `rejected` (ไม่รับดำเนินการ)

**⛔ `closed` (ปิดเรื่อง) เลิกใช้แล้ว 2026-09-04** — ซ้ำกับ `resolved` (ทีมใช้ `closed`+"แก้ไขสำเร็จ" เป็นท่าปิดเคสมาตรฐาน 66 ครั้ง/วัน ส่วน `resolved` ใช้ 4 ครั้งตลอดกาล) และเคยใช้ list เหตุผลชุดเดียวกับ `rejected` จนเลือกข้ามความหมายได้
→ ย้าย `closed`+"แก้ไขสำเร็จ" 65 ใบไป `resolved` แล้ว · เหลือ ~16 ใบที่เหตุผลเป็นการปฏิเสธ **คาไว้เป็น `closed` โดยตั้งใจ** (ไม่แก้ป้ายสถานะสาธารณะย้อนหลังให้ผู้ร้องที่เคยเห็นแล้ว)
→ ยังอ่านได้ทุกที่ (`STATUS_LABELS`, `DONE_STATUSES`, ตัวกรองในหน้ารายการ) แต่ **เลือกใหม่ไม่ได้** — `SELECTABLE_STATUSES` ใน `web/lib/caseOptions.js`

**กฎการจบเคส:** ทั้ง `resolved` และ `rejected` ต้องมี **ข้อความแจ้งผู้ร้องเรียน** เสมอ (`rejected` ต้องเลือกเหตุผลด้วย) — เพราะ `/complaint/[ref]` คือช่องทางเดียวที่ผู้ร้องรู้ผล
