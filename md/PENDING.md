...
1. **เทสบน dev ไม่ได้เลย** — `case_config` บน dev มี **0 แถว** (เช็คจริง 2026-08-28) สคริปต์ต้องใช้
   config นี้ → รันที่นี่ไม่ผ่าน · แปลว่า **ไปลง prod ตรงๆ ครั้งเดียว ไม่มีรอบซ้อม**
   → ต้องพึ่ง `--dry-run` + `--limit` น้อยๆ ให้หนักกว่าเดิม
