...
3. **`/kanban` — ✅ ไม่ล้นแล้ว แต่ยังค้าง 2 อย่าง**
   - เลขในวงเล็บของ dropdown "แสดง" (`scopeCounts` + props `t`/`label`/`counts`) **ยังไม่ได้ต่อ**
     เพราะตอนนั้นมี session อื่นแก้ `KanbanHome.jsx` พร้อมกัน (user สั่งให้หยุดรอ 2026-08-31)
   - panel `absolute w-64/w-72` 5 จุดยังไม่ได้ใส่ `max-w-[calc(100vw-1.5rem)]`
     **ที่ 375px ยังไม่ล้น แต่ที่ 320px เมนู "เรียงลำดับ" (`w-64` + `right-0`) ล้นแล้ว 18px** — ไม่ใช่ระเบิดเวลาแล้ว
   - ที่ 320px ยังมี `<input>` ในโมดัลการ์ดล้น 4px (จุดเล็ก · ยังไม่ไล่ เพราะสเปกหลักคือ 375)
