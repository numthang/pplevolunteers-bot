...
## 🛑 หยุดอ่านก่อน — branch `org-core` อยู่ในสภาพครึ่งๆ **ห้าม deploy** (2026-07-22)

**ORG_ACCESS_REDESIGN ขั้น 4 สลับ "ทางอ่าน" ไป `org_member_roles` แล้ว แต่ "ทางเขียน" ยังเขียนที่เดิมทั้งหมด**

→ ไม่มีอะไรเขียนลง `org_member_roles` เลยนอกจาก migration ที่รันครั้งเดียว
→ **สิทธิ์ทุกคนถูกแช่แข็งไว้ ณ ตอน migrate** · ตั้งยศใหม่ไม่ขึ้น · ถอดยศไม่หลุด · ทั้งทางเว็บและทาง Discord

| ทางเขียน | เขียนลง | มีผลต่อสิทธิ์ |
|---|---|---|
| `/api/admin/roles` | Discord + `org_members.roles` | ❌ |
| `/api/org/appoint` | เหมือนกัน | ❌ |
| บอทซิงค์ยศ (`db/members.js`) | `org_members.roles` | ❌ |

**ต้องเลือกทางใดทางหนึ่งก่อนทำอย่างอื่น:**
- **ทำขั้น 5 ต่อ** — ให้ 3 จุดข้างบนเขียน `org_member_roles` ด้วย (ยังเขียน `roles` ไว้เป็นสำเนา) · แนะนำทางนี้
- **ถอยขั้น 4** — `git revert d90cb91` ระบบกลับมาทำงานเต็มร้อยทันที แล้วค่อยทำ 4+5 พร้อมกัน

> บทเรียน: ขั้น 4 (สลับทางอ่าน) กับขั้น 5 (ย้ายทางเขียน) **แยกกันไม่ได้** ต้องไปด้วยกัน

**เรื่องค้างที่ 2 — หน้าจัดการยศซ้ำซ้อน:** `/admin/roles` (16 ก.ค.) ถูกแทนที่ด้วย `/org/settings/members` (17 ก.ค.) แล้ว แต่ยังไม่ถูกลบ · **ไม่มีลิงก์ในเมนู** ต้องพิมพ์ URL เอง · หน้าใหม่ทำได้ทุกอย่างและดีกว่า (พูดภาษาตำแหน่งแทนยศ Discord · คุมทั้ง org · มี FLOOR + ตั้งได้ว่าใครแต่งตั้งได้)
→ เสนอลบทิ้ง จะได้แก้ทางเขียนที่เดียวจบ · ถ้าไม่ลบ ต้องแก้ทางเขียน 2 ที่
