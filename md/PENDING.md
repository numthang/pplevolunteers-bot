...
## 👤 การ์ดโปรไฟล์คนแบบ Discord (จด 2026-08-19 · user สั่ง "เก็บไว้ก่อน ทำ session หน้า")

**ที่มา:** ในการ์ดการบ้าน ชื่อ "เจ้าภาพ / คนช่วย" กดแล้วเปิดกล่องเลือกคน — user คาดว่ามันควรเป็น**ลิงก์ไปหาคนคนนั้น** ไม่ใช่ปุ่มแก้ค่า
ตอนนี้ทั้งระบบ**ไม่มีหน้าโปรไฟล์คนเลย** — ไม่มีที่ให้ลิงก์ไป

**สิ่งที่ user อยากได้ (คำเดิม):** กล่องลอยแบบดิสคอร์ด — เปิดมาเห็น info คร่าวๆ + **มีกล่องแชทส่ง DM ไปหาคนนั้นทางดิสคอร์ดได้จากในเว็บ**

**สิ่งที่ต้องเคาะก่อนลงมือ:**
- กดชื่อแล้วเปิดกล่องลอย แล้ว "แก้ว่าใครเป็นเจ้าภาพ" ย้ายไปกดตรงไหน (ตอนนี้กดที่เดียวกัน — ชนกันแน่)
- ในกล่องโชว์อะไรบ้าง: รูป (`users.avatar` / `org_members.avatar`) · ชื่อทุกแบบ · ยศใน org · การ์ดที่ถืออยู่กี่ใบ?
- **DM ผ่านบอท = ของจริงที่ต้องคิดเรื่องสิทธิ์** — ใครส่งหาใครได้ · ปิดรับ DM แล้วต้องบอกยังไง · เก็บ log ไหม
  (บอทส่ง DM ได้อยู่แล้ว แต่ต้องมี API ฝั่งเว็บ → บอท ซึ่ง**ยังไม่มีเส้นนี้เลย**)
- เป็นกล่องลอยเฉยๆ หรือมีหน้าจริง `/org/people/[id]` ด้วย (deep link / แชร์ลิงก์ได้)

**เกี่ยวข้อง:** สูตรชื่อคนย้ายมาอยู่ที่ `web/db/displayName.js` แล้ว (2026-08-19) — กล่องนี้ต้องใช้สูตรเดียวกัน
