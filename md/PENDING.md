...
**ยังไม่เคาะว่าจะทำเมื่อไหร่** — โผล่มาจากคำถาม "override_data กับ docs_self_info ต่างกันยังไง"
...
### ยังไม่เคาะ
- ชั้น 2 อยู่ที่ `users` (คอลัมน์ตรงๆ) หรือตารางใหม่ key `(user_id, org_id)`
  - `users` = ง่ายกว่า ตรงกับ `firstname`/`lastname`/`id_card_image` ที่อยู่ที่นั่นแล้ว **แต่แชร์ข้ามองค์กร**
    (มี precedent วิธีกัน: `/api/docs/id-card/[userId]` เช็ค "ต้องอยู่ org เดียวกัน" ก่อนเสิร์ฟรูป)
  - `(user_id, org_id)` = แยกขาดต่อองค์กร แต่เพิ่มตารางใหม่
- **ห้ามเลือก `org_members`** — นั่นคือการจ่ายหนี้ก้อนเดิมเพิ่ม
- ต้องแตะหน้า `/profile/settings` ด้วย ไม่ใช่แค่ docs (ไม่งั้นได้ที่เก็บที่ 4)
