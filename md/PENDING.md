...
### ยังไม่ได้ตรวจ (ไล่ต่อได้เลย session หน้า)

- [ ] **`createEntries` (`web/db/docs/entries.js:122`)** — INSERT `override_data` ตอนสร้างใบ ยังไม่ได้ดูว่าทับของเดิมไหมตอนสร้างซ้ำ/สร้างทับ
- [ ] **`DocAutoCalc` สร้างใบเป็นชุด** — ส่ง `overrideData` หลายใบพร้อมกัน ยังไม่ได้ไล่ว่าเคสไหนไปทับของผู้รับ
...
- [ ] **`bug-452` หน้าตัน** — flexible + คนเซ็นแทนที่ไม่ใช่ผู้ดูแล + ข้อมูลผู้รับไม่ครบ → `canSign=false` แต่ไม่มีการ์ดไหนขึ้นเลย ไม่บอกเหตุผล
- [ ] **`description` ของใบที่เซ็นไปแล้ว** — แก้ระยะทางหลังเซ็นยังเปลี่ยน description ได้ ทั้งที่ลายเซ็นติดอยู่แล้ว
- [ ] **ล็อก `override_data` หลังเซ็น** — ตอนนี้ยังแก้ได้และ PDF สร้างสดทุกครั้ง = ใบที่เซ็นแล้วเนื้อหาเปลี่ยนย้อนหลังได้ (จดไว้ตั้งแต่ §🧬 org_members ยังไม่ทำ)
