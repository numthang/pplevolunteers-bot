...
1. ~~**การ์ดสำเนาบัตรในหน้าเซ็นไม่โผล่ให้ผู้ดูแล**~~ ✅ **แก้แล้ว 2026-08-26 (local · ยังไม่กดทดสอบจริง)**
   `/api/docs/sign/verify` คืน `can_manage` (เช็ค `canManageDocs` + `canAccessEvent` ด้วยกฎเดียวกับ gate ของ `POST /api/docs/entries/:id/id-card` เป๊ะ) → หน้าเซ็นโชว์การ์ดเมื่อ *เจ้าตัว **หรือ** คนนอก **หรือ** (จัดการเอกสารได้ **และ** โหมดยืดหยุ่น)*
   ผู้ดูแลไม่ต้องผ่านขั้นผูกทะเบียน/กรอกเอง (เขากรอกให้ในใบอยู่แล้ว) · อัปโหลดวิ่งเข้า `entries/:id/id-card` ไม่ใช่ `/api/docs/id-card` (ไม่งั้นทับบัตรแอดมินเอง) + ถาม confirm ก่อนทับของเดิม
