...
1. ↩️ **กลับคำอีกรอบ 2026-09-03 (user สั่งเอง): backfill `post_assignees` จาก `created_by` แล้ว**
   ...
   และไม่ดันการ์ดไปกองไหนแล้วตั้งแต่ถอดกฎ "ชื่อคน↔กอง" (ดู `md/kanban/KANBAN.md`)
...
## 🔜 mobileAudit รอบ 2 — ตรวจ "เบียดกันเหมือนตาราง" กับ "ปุ่มไม่ fluid" (prompt สำหรับ session หน้า · ตั้งโจทย์ 2026-09-19)

**ต้นเรื่อง:** user ทัก "mobileaudit ยังไม่ค่อยแม่น … บางครั้งไม่ใช่แค่ล้นหน้าจอ แต่แบบวางเรียงกันจนเบียดข้อความกัน
เหมือนตาราง บน mobile" + "ไหนจะเรื่องปุ่มไม่ fluid อีก" · หน้าตัวอย่างที่เป็นอยู่จริงตอนนี้:
`/finance/payouts` (การ์ดรอบ) และ **`/finance/payouts/1` (แถวรายชื่อ + แถวปุ่มท้ายหน้า)**

### จุดบอดที่ยืนยันแล้วในโค้ดปัจจุบัน

1. **`truncate` ถูกยกเว้นทั้งหมด** — `scripts/dev/mobileAudit.mjs:261`
   ```js
   if (m.cs.textOverflow === 'ellipsis') continue   // truncate = ตั้งใจตัด มี … ให้เห็นอยู่แล้ว
   ```
   → ชื่อรอบที่เหลือ 100px จาก 380px ก็ "ผ่าน" เพราะมี `…` · **นี่คืออาการที่ user เห็นว่าเบียด**
   ข้อสมมติเดิมผิดตรงที่ "ตั้งใจตัด" ≠ "ตัดเท่าไหร่ก็ได้"
2. **กฎ B/C/D วัดเทียบขอบจอเท่านั้น** — การเบียดเกิด*ในแถว* ไม่ได้ล้นขอบจอ เลยไม่มีกฎไหนจับ
3. **กฎ F (แถวปุ่มไม่เต็มความกว้าง)** เป็นคำแนะนำล้วน + `mobileAudit.mjs:287` ข้ามแถว
   `justify-between/around/evenly` → แถวปุ่มท้าย `/finance/payouts/1` ที่ปุ่มกว้างไม่เท่ากันและขอบขวาแหว่ง ไม่ถูกจับ

### กฎที่เสนอให้เพิ่ม (ตัวเลขปรับได้ แต่ต้องมีเหตุผลกำกับ)

| รหัส | อาการ | สูตรวัด | ระดับ |
|---|---|---|---|
| **N** | ข้อความโดนตัดจนอ่านไม่รู้เรื่อง | `textOverflow === 'ellipsis'` **และ** (`clientWidth / scrollWidth < 0.55` **หรือ** `clientWidth < 120px`) | **error (exit 1)** |
| **P** | คอลัมน์ข้อความถูกบีบในแถว | แถว flex แนวนอนที่มีลูก ≥3 ชิ้น และลูกที่เป็น `flex-1/min-w-0` เหลือ < 45% ของความกว้างแถว | คำแนะนำ |
| **Q** | แถวปุ่มแหว่ง/ไม่ fluid | แถว flex-wrap ที่มีปุ่ม ≥2 และ**บรรทัดใดบรรทัดหนึ่ง**กินพื้นที่ < 85% ของแถว **หรือ** ปุ่มในคอลัมน์เดียวกันกว้างไม่เท่ากัน (ต่าง > 8px) ที่ ≤640px | คำแนะนำ |

- **N ต้องเป็น error** ไม่ใช่คำแนะนำ — ไม่งั้นก็จะถูกกลืนไปกับ 6 บรรทัดคำแนะนำเดิมแล้วไม่มีใครแก้
- ทุกกฎต้องรายงาน **ตัวเลขจริง** ("ชื่อรอบเหลือ 98px จากเนื้อหา 412px = โชว์ 24%") ไม่ใช่แค่ชื่อกฎ —
  คนอ่านต้องตัดสินได้ทันทีว่าเคสนี้ยอมรับได้ไหม โดยไม่ต้องเปิดเบราว์เซอร์เอง
- ต้องมี **ทางยกเว้นรายจุด** (เช่น `data-audit-ok="truncate"`) สำหรับข้อความที่ยาวเกินจะพอดีจริงๆ
  ไม่งั้นจะได้ false positive จนคนปิดสคริปต์ทิ้ง

### เคสทดสอบที่รู้คำตอบอยู่แล้ว (ใช้เช็คว่ากฎใหม่จับได้จริง)

- `/finance/payouts` — ชื่อรอบ "ผู้บริหารท้องถิ่นและระบบเรื่องร้องเรียน" โดนตัดเหลือ ~1/3 → กฎ **N** ต้องยิง
- `/finance/payouts/1` — แถวรายชื่อ `[☑][ชื่อ+ธนาคาร][🔔][ช่องยอด][🗑]` คอลัมน์ชื่อถูกบีบ + บรรทัดธนาคาร
  โดนตัด ("กสิกรไทย 339…") → กฎ **N** + **P** ต้องยิง
- `/finance/payouts/1` — แถวปุ่มท้ายหน้า (คัดลอก / ดาวน์โหลด CSV / แจ้งทุกคน / ปิดรอบ) กว้างไม่เท่ากัน
  ขอบขวาแหว่งทุกบรรทัด → กฎ **Q** ต้องยิง
- **ต้องมีหน้าที่ต้อง "ไม่" ยิงด้วย** (regression guard) — `--all` แล้วเทียบว่าไม่มีหน้าไหนกลายเป็น error ใหม่
  เกินกว่า 3 หน้าที่ตั้งใจ ถ้าท่วมแปลว่าตั้ง threshold แน่นไป

### ชั้นที่สอง: ให้คนดูภาพ (อย่าพยายามให้เครื่องตัดสินความสวย)

เครื่องตัดสินได้แค่ "ตัวเลขผิดปกติ" · ส่วน "เรียงแล้วดูเป็นตารางอัดๆ" เป็นการตัดสินด้วยตา
→ `mobileAudit --shot` มีอยู่แล้ว และ Claude อ่านภาพ PNG ได้ · ให้ session หน้าเชื่อมกับ `/designqc`
ว่า **ถ้ากฎ N/P/Q ยิง ให้ถ่ายรูปหน้านั้นแล้วอ่านภาพประกอบเสมอ** ก่อนสรุปว่าแก้ตรงไหน

### สิ่งที่ต้องแก้ตามมาหลังกฎใหม่ผ่าน (งาน UI จริง ไม่ใช่งานสคริปต์)

- `/finance/payouts/1` แถวรายชื่อ: ที่ ≤640px ควรแตกเป็น 2 บรรทัด (ชื่อ+ธนาคารเต็มบรรทัดบน · ยอด+ปุ่มบรรทัดล่าง)
  แทนการยัด 5 ช่องในแถวเดียว
- แถวปุ่มท้ายหน้า: `w-full sm:w-auto` หรือ `grid grid-cols-2 sm:flex` ให้เต็มความกว้างบนมือถือ
- ⚠️ ห้ามแก้ UI ก่อนกฎใหม่เสร็จ — ไม่งั้นไม่มีเคสทดสอบที่รู้คำตอบเหลือไว้เช็คกฎ
