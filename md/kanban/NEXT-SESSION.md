...
**17 commit บน `master` เครื่อง dev — ยังไม่ push ยังไม่ deploy** (`bc44bf5` … `253ea71`)
...
### 4. ⬜ ติดตั้ง ESLint (user ยังไม่เคาะ)
โปรเจกต์**ไม่มี ESLint เลย** → `next build` จับ ReferenceError ตอน render ไม่ได้
(เจอจริง 2026-08-19: ลบฟังก์ชันพลาดแล้ว build ผ่าน user เปิดหน้าถึงเจอ — `253ea71`)
