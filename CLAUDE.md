...
## 🗄️ Query ต้องอยู่ใน db/ เท่านั้น (เคาะ 2026-09-17)

- SQL/`pool.query` อยู่ใน `db/` (บอท) หรือ `web/db/` (เว็บ) เท่านั้น — ที่อื่นให้ import **ฟังก์ชัน** จาก `db/<module>` / `@/db/<module>` · ห้าม import `pg` หรือ pool (`db/index`, `@/db/index.js`) ตรงๆ
- ไฟล์เดิมที่หลุดอยู่ถูกล็อกไว้ใน **`eslint.db-allowlist.mjs`** (root) — **ลบได้อย่างเดียว ห้ามเพิ่ม**
- **แก้ไฟล์ที่อยู่ใน allowlist = ย้าย query ของไฟล์นั้นเข้า db ใน PR เดียวกัน แล้วลบชื่อออกจาก allowlist** (ตัวอย่าง: `web/lib/emailBindLink.js` → `web/db/emailBind.js`)
- บังคับด้วย ESLint (`npm run lint:all` — ชื่อค้างใน allowlist ก็ทำให้ lint พัง) + hook `.claude/hooks/block-direct-db.js` (บล็อก Edit/Write) · ยกเว้น: `db/**`, `scripts/**`, migrations, เทสต์
