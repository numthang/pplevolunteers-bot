...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('ติดตั้ง Sticky Message แนะนำตัวใน channel นี้')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels)
    .addBooleanOption(o => o.setName('sticky').setDescription('เปิดใช้งาน sticky').setRequired(false))
  ...
  async execute(interaction) {
    ...
    const row   = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_open_register_modal').setLabel(buttonLabel).setStyle(ButtonStyle.Primary)
    );
    ...
    await interaction.editReply({ content: `✅ ติดตั้ง Sticky Message เรียบร้อย!\nLog → ${logChannel.id === interaction.channelId ? 'channel นี้' : `<#${logChannel.id}>`}` });
  },
};