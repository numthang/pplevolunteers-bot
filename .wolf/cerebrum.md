...
### kanban: ป้ายถูก query มาให้อยู่แล้ว UI แค่ไม่ได้วาด (2026-08-17)
`web/db/kanban/cards.js` มี `AGG` ที่ json_agg ป้ายมากับทุก query (getCard/listCards/listMyCards) ตั้งแต่ก้อน 1 — อาการ "import แล้วเหมือนไม่มีข้อมูล" จึงไม่ใช่ปัญหา query/import แต่คือ component ไม่ได้ render `card.labels` · เช็ค payload ของ API ก่อนไล่หา bug ฝั่ง DB
สีชิปห้ามผูกกับชื่อกลุ่ม (org ตั้งเอง) → hash ชื่อกลุ่มเลือกช่องในจานสีคงที่ (`web/lib/kanbanLabelColors.js`) และ class ต้องเขียนเต็มสตริง ห้ามต่อ `bg-${c}-100` (Tailwind สแกน static)
...
- **ORG-SWITCHER SPINE — org-first scope, guild ไม่ใช่ anchor (เคาะ 2026-07-17):** feature org-native (finance) scope ด้วย `org_id` · `getOrgId`=active_org cookie (resolveActiveOrg by userId) ไม่ derive จาก guild → guildless self-serve org เข้าถึงได้ · **owner (membership role) = org superuser** → getEffectiveOrgIdentity add 'admin' (bounded org ตัวเอง, แก้ deadlock owner ไม่มีสิทธิ์) · **ไม่ flip getEffectiveIdentity ทั้งระบบเป็น org-keyed** — feature ที่ยัง guild_id-scoped (calling/docs/cases/bot ~50 route) ถ้า access เป็น org-union = elevation (permission guild A ทำ guild B ใน org เดียว) → orgAccess **finance-only** revisit ต่อ feature ตอน migrate · guild-based features ตาม active_org ผ่าน **dual-write selected_guild** ตอน switch (ไม่แตะ getGuildId = blast 0) · guildless org → Nav ซ่อน app guild-based (ORG_NATIVE_FEATURES=['finance']) · client gating: `/api/me/access?scope=org`
...
## Decision Log (2026-08-18 รอบเย็น — kanban custom field: ไม่มี admin gate)
- user ส่งสกรีนช็อตจริงของ AppFlowy 2 ภาพ (checklist + multi_select option picker) กลางที่กำลังเขียนก้อน 2 (5 ชนิดสเกลาร์) แล้วขอเปลี่ยนแผน: เร่ง select/multi_select มาทำพร้อมกัน (เดิมวางไว้ขั้น 3) + checklist กลับคำจาก "คอลัมน์จริง" (เส้นแบ่งเดิมต้นแผน) → เป็น custom field type
- user เห็นว่า AppFlowy จัดการตัวเลือก (rename/สี/ลบ/ลากจัดลำดับ) **ในตัว combobox เอง** แล้วถาม "ด้วยวิธีนี้ไม่ต้องมี field manager ให้ยุ่งยาก ถูกไหม" → ตัดหน้าแอดมิน `/kanban/fields` ทิ้งทั้งหน้า ไม่มี `isKanbanAdmin` เหลือในระบบ custom field เลย (ต่างจาก labels ที่ rename/hide ยังเป็นสิทธิ์ admin) — field/option/checklist ทุกอย่างใช้ `canEditCard` เหมือนกันหมด
- **บทเรียนกระบวนการ:** พลาดจังหวะ commit checkpoint ก่อนเริ่มก้อน 2 (5 ชนิดสเกลาร์) แล้วโดน scope เปลี่ยนกลางทาง — ครั้งนี้กู้ทันเพราะ commit ก้อน 2 เดิมไปก่อนเริ่มต่อ select/multi_select/checklist (ตาม [[feedback_flag_before_crossing_boundary]]) ไม่งั้นต้อง revert งานที่ verify ผ่านแล้วทิ้ง
...
- **NextAuth instance ที่ 2 แยก ไม่ extend `auth-options.js` เดิม** — เพราะ signIn เดิม hard-block non-Discord ด้วย `NotLinked` (auth-options.js:78-81); แยก provider เดียวไม่ได้ว่ามาจากฝั่งไหน → extend = แตะ path login PPLE prod = เสี่ยง. ปลายทางคือ converge เป็น 1 (Phase 3+) ไม่ใช่ 2 ตลอด — ของเก่าถูกดูดรวม ไม่ทิ้ง

## Key Learnings (2026-07-15 — two-instance NextAuth v4)
- **`__NEXTAUTH` เป็น module-global** ใน next-auth/react v4 → SessionProvider สอง basePath ชนกัน (ตัว mount ทีหลังทับ global). วิธีเลี่ยง: **platform pages = server component** (getServerSession(orgAuthOptions)) + trigger login แบบ manual CSRF (fetch `/csrf` → POST `/signin|/callback/<provider>` พร้อม header `X-Auth-Return-Redirect:1`, body `json=true`) — ดู `web/lib/orgSignIn.js`. ไม่ต้องพึ่ง next-auth/react client เลย
- **cookie namespace ต้องแยกครบชุด** (session/csrf/callback/pkce/state/nonce) ไม่ใช่แค่ session — ตั้ง prefix `org-auth.*` ใน `cookies:` ของ orgAuthOptions ไม่งั้น OAuth dance ชน cookie PPLE
- **NextAuth EmailProvider ต้องมี DB adapter** (เราเป็น JWT ไม่มี adapter) + email transport → เลี่ยงด้วย reuse pattern nonce→CredentialsProvider เดิม (เหมือน passkey/phone): token เก็บ `org_login_tokens` (email-keyed เพราะ dc_user_config PK = discord_id,key ใช้ไม่ได้)
- **email identity ต้อง partial-unique** `uq_dc_members_email ON dc_members(email) WHERE email IS NOT NULL` — กัน invite(shell)+login+multi-org สร้าง dup identity. findOrCreate ใช้ `ON CONFLICT (email) WHERE email IS NOT NULL`
- **Phase 0 ปลด NOT NULL ต้องรวม `username`** ด้วย (ไม่ใช่แค่ discord_id/guild_id) — email/shell row ไม่มี username. คง DEFAULT '' ของ guild_id ไว้ แล้ว insert NULL explicit (ไม่แตะพฤติกรรม PPLE insert)
...
- `post_publish_jobs` = **1 แถวต่อ (ตอน × แพลตฟอร์ม)** + `batch_id` → retry ไม่โพสต์ซ้ำ (ของเดิมใน basketHandler เก็บ `partial` แล้วไม่ retry เลย)
...
## 📥 merged จาก `md/WOLF-INBOX.md` (2026-08-14 · จดบน Mac → ยกเข้าที่นี่บน Linux)
...
## Decision Log (2026-08-18 — kanban import รอบ 2: รอ custom field นิ่งก่อน)

- **อาการที่ user เห็น:** `backups/kanban/kanban_import.xlsx` มี 83 แถว แต่ DB มีแค่ 30 กว่า → **ไม่ใช่บั๊ก**
  `scripts/import/kanbanFromAppflowy.mjs:198` กรอง `LIVE` เป็นค่าตั้งต้น = เอาเฉพาะงานที่ยังไม่จบ 32 ใบ
  (82 แถวมี Title จริง · `เสร็จแล้ว` 50 ใบตั้งใจพักไว้รอ custom field · DB 37 = 32 import + 5 สร้างมือ)
- **เคาะ:** ยังไม่ import ตอนนี้ · รอเคาะ "ยุบป้ายเข้า custom field ไหม" ให้จบก่อน แล้ว import รอบเดียวทั้ง 82 ใบ
  **เหตุผล:** ข้อนั้นตัดสินว่า `อำเภอ`/`category`/`อุปกรณ์` ลงเป็นป้ายหรือ field — import ก่อน = ย้าย 84 เส้นทีหลังอยู่ดี
- **user เป็นคนชี้เองว่าต้องเช็คตัวซ้ำ** — สคริปต์ไม่มี dedupe เลย `--all` วันนี้ = 82 ใบทับของเดิม 32 ใบ
- **`Checklist` จาก AppFlowy กู้ไม่ได้ถาวร** — export มาเป็นตัวเลข % (`0.73`) ไม่มีตัวข้อความ ไม่เกี่ยวกับ custom field
- รายละเอียดเต็มอยู่ `md/kanban/NEXT-SESSION.md` §4
...
- **ถอดธง "ติดปัญหา" (`blocked`) ออก** — user: *"ยังไม่ได้ใช้ ถ้าใช้เดี๋ยว custom field เอา"*
  เหตุผลที่หนักที่สุด: **มันพังมาตั้งแต่ก้อน 1 แล้วไม่มีใครบ่นเลย** = หลักฐานว่าไม่มีใครใช้
  คอลัมน์ใน DB เก็บไว้ (ย้อนได้ ไม่ต้อง migration)
...
## Key Learnings (2026-08-19 — ยุบป้ายเข้า custom field)

- **ระบบ "ป้าย" ตายแล้ว** — `kanban_labels`/`kanban_card_labels` ยังมีข้อมูลอยู่แต่ **ไม่มีโค้ดอ่านแล้ว**
  แท็กทั้งหมด = ตัวเลือกใน custom field · ทางเขียนจุดเดียวคือ `web/db/kanban/tags.js`
  (`ensureTagField` / `ensureTagOption` / `ensureTag` / `addCardTags`)
- **`cardTags(card)` ใน `lib/kanbanTagFilter.js` = จุดแปลง field → ชิป จุดเดียวของระบบ**
  ทั้งชิปบนการ์ดและชิปตัวกรองดึงจากตรงนี้ · เขียนซ้ำที่อื่น = ชิป 2 ที่หลุดจากกัน
  checklist **ไม่** นับเป็นแท็ก (เป็นงานย่อย ไม่ใช่หมวด)
- **`kanban_field_defs.key` = `field_<id>` → ผูกกับ id ที่เครื่องนั้นสร้าง** dev กับ prod ได้คนละเลขแน่นอน
  ⛔ ห้าม resolve field ด้วย id หรือ key ในสคริปต์/migration ใดๆ — ใช้ **ชื่อ (`org_id` + `label`)** เสมอ
- `kanban_field_options` มี unique index `uq_kanban_field_options_name` บน (field_id, name) → ON CONFLICT ได้
- ป้ายหาสีจาก hash `กลุ่ม/ชื่อ` · option หาจาก `field:<id>` → **ชื่อเดียวกันได้คนละสี** ตอนย้ายต้องก็อป color ไปด้วย
