...
## Key Learnings
- **ไฟล์แนบเคสมี 3 ทางเข้าแล้ว (2026-09-13)** — ฟอร์มสาธารณะ (`POST /api/case`, ≤3 ไฟล์/ครั้ง) · sync จากเธรด Discord (`lib/caseAttachmentSync.js`) · **เจ้าหน้าที่แนบเองในหน้าเคส (`POST /api/case/[ref]/attachments`, เพดาน 10 ไฟล์ต่อเคสนับจากของที่มีอยู่จริง)** · allowlist mime อยู่ที่ `lib/caseUploads.js` ที่เดียว (มี `application/pdf` แล้ว) → แก้ที่นี่ = กระทบทั้ง 3 ทางพร้อมกัน ห้ามเพิ่ม mime โดยไม่นึกถึงฟอร์มสาธารณะ
- **ลบไฟล์แนบต้อง unlink เองเสมอ** — `uploads/cases/` ไม่มี gc (ต่างจาก `storage/posts` ที่มี `scripts/posts/gc-media.js`) · ไฟล์ที่ sync มาจาก Discord ลบแล้ว **ไม่กลับมา** เพราะ `cases.last_attachment_message_id` เลื่อนไปข้างหน้าอย่างเดียว ไม่มีทาง reset
- **เสิร์ฟไฟล์ที่ผู้ใช้อัปโหลดจาก origin เดียวกับเว็บ: `nosniff` เสมอ + inline เฉพาะ `image/*`** — PDF/ไฟล์อื่นบังคับ `Content-Disposition: attachment` (`lib/caseUploads.js#isInlineSafeMime`) · กฎนี้ใช้กับทุกโมดูลที่เสิร์ฟไฟล์แนบ ไม่ใช่แค่ cases
- **การ์ด/กล่องที่มีปุ่ม action อยู่ข้างใน ห้ามซ่อนด้วย "จำนวนข้อมูล"** ให้ซ่อนด้วย "สิทธิ์" — การ์ดไฟล์แนบในหน้าเคสเคย `attachments.length > 0 &&` ทำให้เคสที่ยังไม่มีไฟล์ไม่มีที่ให้กดแนบเลย (bug-519) · empty state ที่ถูกคือโชว์การ์ดพร้อมข้อความ "ยังไม่มี…"
- **`useAutoGrow(value)` ยืดกล่องเมื่อ "ค่า" เปลี่ยนเท่านั้น** — textarea ที่ถูก unmount/mount ตามสถานะหน้าจอ (เช่นสลับ step ในโมดัลแล้วย้อนกลับ) ได้ DOM node ใหม่ที่ความสูง default ทั้งที่ค่าเท่าเดิม → **ต้องเอาสถานะนั้นเข้า dep ด้วย** (bug-518)
- **ร่างหนังสือราชการ = งาน writing ไม่ใช่ `task: 'light'`** — เคยปัก haiku ไว้เพราะคิดว่า "ร่างจากเทมเพลต" ผลคือร่างตื้นต้องเขียนใหม่ทุกใบ · และ **context ที่ไม่ได้ส่ง = สิ่งที่ AI จะกุ**: ไม่ส่งรายการไฟล์แนบจริง AI ก็คิดชื่อเอกสารแนบขึ้นมาเอง (แก้แล้วที่ `app/api/case/[ref]/letter/draft/route.js` — ส่งเลขเรื่อง วันที่รับเรื่อง ผู้ร้อง และชื่อไฟล์แนบจริง)
- **custom field ผูกกับ "กระดาน" ไม่ใช่ teamspace — ยืนยันซ้ำ 2026-09-08 (ห้ามเสนอย้ายอีก)** · Claude เสนอให้ย้ายไปผูก teamspace (คลังช่องเดียวต่อทีม · ย้ายการ์ดในทีมแล้วค่าไม่หาย) **user ตีตก**: *"แต่ละบอร์ดมันต้องมี field ไม่เหมือนกัน ไม่งั้นจะแยกบอร์ดทำไม"* — บอร์ด = ชุดข้อมูลคนละแบบ ไม่ใช่แค่การแบ่งกอง · ผลที่ยอมรับแล้ว: ตัวเลือกของแต่ละบอร์ดแยกทางกันได้ (`copyFieldDefs` เป็นแค่ **จุดตั้งต้น**ตอนสร้างบอร์ด ไม่ใช่การ sync) และย้ายการ์ดข้ามบอร์ดแล้วค่าหายจากจอแม้อยู่ทีมเดียวกัน
...
### kanban ผูกของจริง — สถานะสด + ด่านการมองเห็น (2026-08-24)

- **`web/db/kanban/statusSql.js` = จุดเดียวที่แปลงสถานะต้นทาง → `status_type` ของ kanban**
  สูตรถูกใช้ 3 ที่ (คอลัมน์ที่ SELECT · WHERE ตอนกรองงานปิด · UPDATE ตอนถอดลิงก์) —
  ก็อปไว้หลายที่แล้วแก้ไม่ครบ = การ์ดโผล่กองหนึ่งแต่หายจากอีกกอง
- **`c.status_type` / `c.title` ของการ์ดที่ผูกของจริงเป็นแค่ cache** อ่านสดจากต้นทางทุกครั้งที่แสดง
  → เปลี่ยนสถานะเคสที่ `/case` แล้วการ์ดขยับกองเองโดยไม่มีใครลาก
  ⚠️ `listCards`/`listMyCards` ต้องกรอง `includeClosed`/`status` ด้วย **สูตรสด** ไม่ใช่คอลัมน์
- **ด่านการมองเห็นข้ามระบบ: kanban เปิดทั้ง org แต่ต้นทางไม่ได้เปิดขนาดนั้น**
  เคสกรองจังหวัด + ต้องมียศ `manageCases` (`db/cases.js:listCases`) · โพสต์ `personal` = ของเจ้าของคนเดียว
  → `visibleLinkSql()` **fail closed**: ไม่ส่ง viewer มา = ซ่อนการ์ดที่ผูกเคสทุกใบ
  → `getCard()` (ภายใน หลัง mutation) แยกจาก `getCardForViewer()` (ตอบ request) — guard ใช้ตัวหลังเสมอ
- **`kanban_card_links` FK ได้ข้างเดียว** (`card_id`) เพราะ entity ชี้ได้ 2 ตาราง
  → ฝั่ง entity ต้องกวาดด้วยโค้ด: `deletePost` ลบถาวรจริง ถ้าไม่ลบการ์ดตามจะเหลือการ์ดกำพร้า
  เคส**ไม่มี** hard delete จึงไม่มีปัญหานี้
- **auto-mirror ต้องมีทั้ง hook และตัวกวาด** — ทางสร้างเคส/โพสต์มีหลายทาง (เว็บ · บอท · สคริปต์ import)
  แขวน hook ให้ครบทุกทางยังไงก็พลาด → `reconcileEntityCards()` คือตาข่ายที่ทำให้ "ครบทุกใบ" เป็นจริง
- **การ์ด mirror ห้ามไม่มีเจ้าภาพ** — `isMyCard()` นับงานที่ไม่มีเจ้าภาพเป็น "ของทุกคน"
  เคส 200 ใบไม่มีเจ้าภาพ = หน้า "การบ้านของฉัน" พังทั้งทีม → ลากเจ้าภาพจากต้นทางเสมอ
  (เคส = `case_assignees` คนแรก · ที่เหลือลงเป็นคนช่วย · โพสต์ = `owner_user_id`)
- **`kanban_cards.created_by` เป็น NOT NULL แต่ต้นทางอาจไม่มีคนสร้าง** — เคสจากฟอร์มสาธารณะ
  `cases.created_by` เป็น null → สำรองด้วย `created_by` ของกระดานแรกของ org


### kanban — ตัวกรองโกหก: ต้นทางมี 2 ไม่ใช่ 1 (2026-08-24 รอบสอง)

- **LIMIT + จัดเรียงฝั่ง client = คำโกหก 2 ชั้น** ที่ต้องแก้พร้อมกันเสมอ
  `listCards` ตัดด้วย `ORDER BY due_at` **ตายตัว** แต่ผู้ใช้เลือกเรียงเองได้ (`lib/kanbanSort.js`
  เรียงใน JS ล้วน รวม custom field) → ย้าย**แต่ตัวกรอง**ไป SQL = "ไม่พบ" หายไป แต่ "เรียงแล้ว"
  กลายเป็นคำโกหกแทน และจับได้ยากกว่ามาก (ผลลัพธ์ดูสมเหตุสมผล แค่ขาดใบที่ควรอยู่หัวตาราง)
  → **เกณฑ์ตัดสิน: ตัวกรองกับตัวเรียงต้องอยู่ฝั่งเดียวกันเสมอ** จะ client ทั้งคู่ หรือ SQL ทั้งคู่
- **ทางที่เลือกคือเอา LIMIT ออก ไม่ใช่ย้ายไป SQL** — วัดจริงก่อนตัดสิน: การ์ดใบละ ~1.5KB
  (วัดด้วย `octet_length(row_to_json())` บน dev) · ของจริงเต็มที่ ~1,500 ใบ ≈ 2MB → ไหวสบาย
  เหลือ `CARD_HARD_CAP = 3000` เป็นเพดานกันระเบิดที่ไม่มีวันชน + ธง `truncated` บอกผู้ใช้ถ้าชนจริง
- **เทคนิค: ดึง `limit + 1` แล้วเทียบ `rows.length > limit`** — เทียบ `=== limit` เฉยๆ จะเตือนผิด
  ตอนมีข้อมูลพอดีเป๊ะเท่าเพดาน (เตือนว่า "ไม่ครบ" ทั้งที่ครบ = โกหกอีกทาง)
- ⛔ **ห้าม import ค่าคงที่จาก `web/db/**` เข้า client component** — ไฟล์พวกนั้น import `pg`
  จะลาก pg เข้า bundle · ที่นี่แก้ด้วยการใช้ `cards.length` แทน (ตอน truncated มันเท่าเพดานพอดี)


### ตะเข็บบอท↔เว็บ: เลือก "เขียน CJS คู่" ไม่ใช่ import ESM ข้ามฝั่ง (2026-08-25)

- โปรเจกต์นี้มี 2 ฝั่งที่เขียน DB เดียวกัน: บอท (CJS, `db/*.js`) กับเว็บ (ESM, `web/db/**`)
  ฝั่งบอทจะเรียกของฝั่งเว็บได้ต้อง `await import()` ซึ่ง**ลาก `web/db/index.js` = pool ที่สองเข้าโปรเซสบอท**
  บอทรันค้างตลอดเวลา → คอนเนกชันคูณสอง · **สคริปต์ `.mjs` ยอมได้เพราะรันจบแล้วตาย บอทไม่ได้**
  → กติกา: **ฝั่งบอทเขียนคู่แฝด CJS เสมอ** แล้วแปะคอมเมนต์ชี้ไฟล์คู่ไว้ทั้ง 2 ฝั่ง
  (ตะเข็บที่มีแล้ว: `db/kanbanCards.js` ↔ `web/db/kanban/{cards,links}.js` ·
   `db/mediaBasket.js` ↔ `web/db/posts/basket.js` · `db/aiCreds.js` ↔ `web/lib/aiCreds.js`)
- **สูตรชื่อสำรองต้องตรงกันเป๊ะทั้ง 2 ฝั่ง** — เคสที่เกิดจากบอทกับจากเว็บต้องได้ชื่อการ์ดแบบเดียวกัน
  (`title || 'เรื่องร้องเรียน ' + ref`) ไม่งั้นของอย่างเดียวกันหน้าตาต่างกันแล้วแต่ว่าใครสร้าง

### Discord API: `before=` ต้อง encodeURIComponent (2026-08-24)

- `archive_timestamp` เป็น ISO8601 ลงท้าย `+00:00` → ส่งดิบลง query string `+` = ช่องว่าง → **400**
  แล้ว loop ดึงกระทู้ `break` ออกกลางคัน · **ไม่ crash แค่ได้ข้อมูลไม่ครบ** (forum เดียวกัน 109 vs 481)
- บทเรียนกว้างกว่านั้น: **สคริปต์ดึงข้อมูลที่ `break` เมื่อ error = ข้อมูลหายเงียบ**
  ถ้าจะ break ต้องพิมพ์ให้ชัดว่า "ดึงได้ไม่ครบ" ไม่ใช่ `[warn]` บรรทัดเดียวแล้วรายงานยอดเหมือนปกติ

### เครดิต AI: Console ≠ subscription · เช็คก่อนแนะนำให้เติมเงิน (2026-08-25)

- `ANTHROPIC_API_KEY` ใน `.env` = บัญชี **Anthropic Console** (จ่ายตามใช้) **คนละบัญชีกับ Claude Code/Max**
  ที่ user จ่ายรายเดือน · เครดิตที่ user เห็นเหลือในแอปเอามาจ่าย API ไม่ได้
- ⛔ **ห้ามบอก user ให้ "ไปเติมเครดิต" โดยไม่พิสูจน์ก่อน** — user บอกว่าเคยโดนแบบนี้แล้วเติมไปก็ยังใช้ไม่ได้
  วิธีพิสูจน์: `GET /v1/models` (ไม่กินเครดิต) เช็คว่า key ใช้ได้ → `POST /v1/messages` ดู error จริง
  → อ่าน header `anthropic-organization-id` **บอก user ไปเลยว่า org ไหนที่หมด** เพราะบัญชีเดียวมีได้หลาย org
- `/v1/messages/count_tokens` **ก็โดนบล็อกด้วย**เมื่อเครดิตหมด — ประมาณราคาต้องวัดจากจำนวนตัวอักษรจริงแทน
...
- ⛔ **`html { font-size: 18px }` (`app/globals.css:22`) — ทุกคลาส rem ของ Tailwind ใหญ่กว่าเอกสาร 12.5%**
  `w-64` = **288px ไม่ใช่ 256** · `w-52` = 234 · `max-w-5xl` = 1152 ไม่ใช่ 1024 (`sm:`/`md:` เป็น px ไม่โดนผล)
  → คำนวณความกว้างจากเลขในเอกสาร Tailwind = **ผิดทุกครั้ง** (เสียเวลาไล่หาที่ 320px เพราะเรื่องนี้ 2026-09-01)
- **เมนูลอยที่เกาะปุ่มซึ่งไม่ได้อยู่ริมขวาสุดของแถว จะยื่นพ้นขอบซ้ายจอ** — `max-w` ช่วยไม่ได้เพราะมันคุมความกว้าง
  ไม่ได้คุมตำแหน่ง · ทางแก้: กล่องครอบปุ่มเป็น `static sm:relative` + แถวเป็น `relative`
  → มือถือเมนูเกาะขอบแถว จอกว้างเกาะปุ่มเหมือนเดิม (เมนู "เรียงลำดับ" ของ kanban)
- **Tailwind arbitrary value ต้องใช้ `_` แทนเว้นวรรค**: `max-w-[calc(100vw_-_1.5rem)]`
  เขียน `calc(100vw-1.5rem)` = CSS ไม่ถูกต้อง เบราว์เซอร์ทิ้งทั้งบรรทัด**เงียบๆ** (เขียนผิดเองมาแล้ว 6 จุด)
- อาการ `E` (แถวที่มี select/input แล้วเหลือที่ว่างท้ายแถว > 32px = ไม่เต็มความกว้าง) เพิ่มตามที่ user สั่ง
  2026-09-01 — **ไม่นับเป็นข้อผิดพลาด** (ไม่ทำให้ exit 1) เพราะบางแถวตั้งใจไม่เต็ม
- กฎ layout มือถือทั้งชุดเขียนไว้ที่ `md/WEB.md §จอมือถือ` แล้ว (งบความกว้างจริงที่ 375 ≈ **339px**)

- **`truncate` ต้องอยู่บน element ที่เป็น block ของข้อความนั้นเอง** — เอา `<a>`/`<span>` (inline) ไปไว้ใน `<h3 class="truncate">`
  แล้วกล่องของ inline ยังกว้างเท่าข้อความจริง (พ่อแค่ overflow-hidden ทับ) → mobileAudit จับว่าล้นจอ 800px ทั้งที่ตาเห็นปกติ
  · แก้: ใส่ `block truncate` ที่ตัวลิงก์ · คู่กัน: `line-clamp-1` ที่มี URL ยาวต้องมี `break-words` ไม่งั้นตัดกลางคำไม่มี … (2026-09-01)
- **ตัวกรองที่ผสมกันได้ ห้ามแปะจำนวน (n) ต่อตัวเลือก** — เลขจะขัดกันเองทันทีที่ตั้งตัวอื่น (เช่น "ยังไม่มีเจ้าภาพ (12)"
...
- **ห้ามเชื่อ `track.getSettings()` ว่าคือสัดส่วนภาพจริง** (เสียเวลาไปทั้ง session · 2026-09-09)
  ...
  (= สิ่งที่ตาเห็น และที่ `MediaRecorder` อัดจริง) อ่านผ่าน `onLoadedMetadata` + `onResize`
  · ผลพวง: ขอกล้อง `1080×1920` หวังแนวตั้ง **จะได้แนวนอน** — ต้องขอ `1280×720` ถึงได้ 720×1280 แนวตั้ง
  (เว็บแคมคอมติดตั้งตรง ไม่หมุน ได้ตามที่ขอ → constraint เดียวให้ผลคนละแบบ ต้องปรับ UI ตามเฟรมจริงเสมอ)
  · อาการปลายทาง: `object-cover` ครอปจนเห็นแค่หน้า — ดู bug-492
- **เรื่องกล้อง/สื่อบนเครื่องจริง ห้ามสรุปจากการอ่านโค้ดหรือทดสอบข้ามอุปกรณ์** (2026-09-09)
  — session นี้ตั้งสมมติฐานผิดติดกัน 3 รอบ (driver crop → CSS crop → capture crop) เพราะเดาจากโค้ด
  ที่จบเรื่องคือ**ทำหน้าเทียบหลายแบบในหน้าเดียวแล้วให้ user กดจริง** (`/dev/camtest`)
  · deploy ทีละสมมติฐาน = เสียรอบละ ~3 นาที + user ต้องรอ → ยัดตัวเลือกให้ครบแล้ว deploy รอบเดียว
...
- **[2026-08-29] `web/.next` เป็น dir เดียวใช้ร่วมกันทุก `next dev` ไม่ว่าจะรันคนละพอร์ตแค่ไหน — ห้าม `rm -rf web/.next` ถ้ามี `next dev` ตัวอื่นรันอยู่** — เคสจริง: throwaway dev server ที่ตั้งใจให้แยกจาก user ด้วย `PORT=3100` (เพื่อไม่ชนพอร์ต 3000) แต่ build cache ไม่ได้แยกตามพอร์ต — พอสั่ง `rm -rf web/.next` เคลียร์ cache ของตัวเอง ดันลบ cache ที่ user's dev server (:3000) กำลังใช้อยู่จริงไปด้วย → server ค้างทันที (listen อยู่แต่ไม่ตอบ แม้รอ 15 วิ) เหมือนกันกับกฎเดิม (2026-07-28) ที่ห้ามรัน `next build` ขณะ `next dev` รันอยู่ — เข้าใจผิดว่าคนละพอร์ต = แยกกันสนิท ทั้งที่ cache dir เป็น per-project ไม่ใช่ per-port. แก้: เช็ค `pgrep -af "next dev"` ก่อนทุกครั้งที่จะแตะ `.next` — ถ้ามีมากกว่า process ของตัวเอง ห้ามลบ/เขียนทับ ปล่อย cache เก่าทิ้งไว้เฉยๆ พอ (ไม่ต้องเคลียร์ก็ได้ ไม่มีผลต่อผลเทส)
...
### คอมเมนต์ JSX วางผิดที่ = build พัง (2026-08-24)

`{/* ... */}` วางได้เฉพาะตำแหน่งที่วาง **element** ได้ · พลาดซ้ำ 2 รอบใน commit เดียว:
1. ระหว่าง prop ในแท็กเปิด → `<KanbanCard {/* c */} draggable={...}` = Syntax Error
2. เป็นตัวแรกใน arrow ของ `.map()` ที่คืน element เดียว → `.map(c => ( {/* c */} <Card/> ))` = คืน 2 ตัว
**ที่ถูก:** วางคอมเมนต์เหนือทั้งก้อน `{shown.map(...)}` ไม่ใช่ข้างใน

### เขียนสถานะกลับลง kanban_cards ต้องเคารพ CHECK owner_required (2026-08-24)

`CHECK (owner_user_id IS NOT NULL OR status_type IN ('backlog','cancelled'))` —
เขียนสถานะสด `done` ลงการ์ดที่ไม่มีเจ้าภาพ = 23514 ทันที (เจอตอน `unlinkCard` กับเคสที่ปิดแล้ว)
ทุกจุดที่เขียน `status_type` ตรงๆ ต้อง clamp ก่อน ไม่ใช่แค่ `setCardStatus`

### ห้ามยุบ field หลายกลุ่มเข้าถังเดียว (2026-08-24)

user จะยุบ "สายงาน" รวมกับ "ป้ายคำค้น" — ทักไว้ว่า **ตัวกรองพัง** เพราะกติกาคือ
**OR ในกลุ่ม · AND ข้ามกลุ่ม** ยุบเป็นถังเดียวแล้ว "งานทีมสื่อ**ที่**ราชบุรี" หาไม่ได้ (กลายเป็น OR หมด = กว้างขึ้น)
· user เลยเปลี่ยนเป็น **รีเนม** field 31 → "ป้ายคำค้น" แทน (key ยัง `field_31` ข้อมูล 67 ใบไม่กระทบ)
⚠️ field "ป้ายคำค้น" ที่ user เพิ่งเพิ่ม **อยู่บน prod ไม่ใช่ dev** — dev มี 5 field เท่านั้น


### บอร์ด kanban ต้องเป็นกล่องเปล่า ห้ามผูกมิติทางภูมิศาสตร์ลง schema (2026-08-24)

user เสนอ use case *"บอร์ดอาจแบ่งตามจังหวัด เพราะนี่คืองานราชบุรีหมดเลย"* แล้ว**ย้ำเองว่าไม่ให้ทำ** —
*"ผมจะไม่ทำให้กรองด้วยจังหวัด แค่บอก use case เฉยๆ ระบบนี้มันต้องทำให้กลางๆ"*

- ⛔ **ห้ามเพิ่มคอลัมน์ `province` / `district` / `region` ลง `kanban_boards`** (หรือตารางไหนใน kanban)
  บอร์ดเป็นกล่องเปล่าที่ org ตั้งชื่อเอง → อยากได้บอร์ด "ราชบุรี" ก็ตั้งชื่อแบบนั้น ไม่ต้องแก้โค้ด
  org อื่นแบ่งตามเขต/ภาค/ไม่แบ่งเลย ใช้กลไกเดียวกัน · ตรงกับ [[project_discord_optional_goal]]
  ที่ระบุว่า geography hardcode ของ PPLE คือหนี้ที่ยังไม่ปลด — อย่าสร้างเพิ่ม
- ข้อมูลจริงหนุนอีกทาง: field "พื้นที่" ของทีมเป็น **อำเภอ ไม่ใช่จังหวัด**
  (ทีมจังหวัด 18 · จอมบึง 9 · บ้านโป่ง 8 · โพธาราม 7 · เมือง 6 …) และ **16/39 ใบ (41%) ติดมากกว่า 1 พื้นที่**
  → มิติภูมิศาสตร์ก็เป็น multi-value เหมือนสายงาน แบ่งบอร์ดตามนี้เจอกำแพงเดิม
  ส่วนระดับจังหวัดตอนนี้เป็นราชบุรีทั้งหมด แบ่งวันนี้ได้บอร์ดเดียวอยู่ดี

### "สถานะเป็นของต้นทาง" ≠ "ต้นทางมีสถานะครบทุกช่วง" (2026-08-25)

เจอตอน user ถามว่าทำไมการ์ดงานสื่อลากไม่ได้ และทำไมกอง "รอทำ" ว่างเปล่าตลอด

- **อาการ:** งานสื่อ 31/31 ใบตกกอง "กำลังทำ" ตั้งแต่วินาทีที่การ์ดเกิด
- **ต้นเหตุที่มองข้ามกันมาตลอด:** `post_episodes.status` มีแค่ `draft/review/approved`
  ซึ่งเป็นสถานะ **บรรณาธิการ** ล้วนๆ — ไม่มีคำว่า "ยังไม่มีใครลงมือ" อยู่ในคำศัพท์ของมันเลย
  ตาราง mapping เลยไม่ได้ *แม็ปผิด* แต่ **ต้นทางไม่มีข้อมูลนั้นให้แปลงตั้งแต่แรก**
- **บทเรียนที่เอาไปใช้ที่อื่นได้:** ก่อนเขียนตารางแปลงสถานะระหว่าง 2 ระบบ ให้ถามว่า
  *"ปลายทางมีช่องไหนที่ต้นทางไม่มีคำให้เลย"* ถ้ามี ช่องนั้นคือช่องที่ **ระบบปลายทางถือเอง**
  (คืน `NULL` ให้ `COALESCE` ตกไปใช้คอลัมน์ของตัวเอง) ไม่ใช่ยัดค่าที่ใกล้เคียงที่สุดลงไป
  — ยัดเมื่อไหร่ ช่องนั้นตายทั้งช่องโดยไม่มีใครสังเกต

⛔ **ห้ามยุบคำศัพท์สถานะของ posts/cases ให้เท่ากับ kanban** (user เสนอ ประเมินแล้วไม่เอา)
`cases.rejected` = ไม่รับเรื่อง → ผู้ร้องได้รับแจ้ง + หน้าติดตามสาธารณะแสดงผล ·
`posts.approved` = ผ่านด่าน `canApprove()` · `posts` "เสร็จ" ไม่ได้อยู่ในคอลัมน์เลย (ดูจาก
`post_social_history`) — ความต่างพวกนี้ **มีภาระจริง** ยุบแล้วต้องเพิ่มคอลัมน์เหตุผลกลับมา
= ความซับซ้อนเท่าเดิม แค่ย้ายที่ + ต้องแก้ read site 43 (posts) / ~15 (cases) + ผูก 3 โดเมนติดกันถาวร
**ที่ควรยุบคือคำที่ *แสดง* ไม่ใช่คำที่ *เก็บ*** ซึ่ง `statusSql.js` ทำอยู่แล้วเป็นจุดเดียว

### `backlog` ของ kanban = `owner_user_id IS NULL` ไม่ใช่สถานะอิสระ (2026-08-25)

บังคับไว้ 3 ชั้น: `setCardStatus` (ย้ายมา backlog = ถอดเจ้าภาพ) · `setCardOwner`
(ตั้งเจ้าภาพ = ออกจาก backlog) · DB CHECK `kanban_cards_owner_required`

**ผลที่ต้องระวังทุกครั้งที่จะให้ "อะไรบางอย่าง" ตัดสินว่าการ์ดอยู่ backlog ไหม:**
ถ้าตัวตัดสินนั้นไม่ใช่ `owner_user_id` ของการ์ดเอง จะได้เจ้าภาพ 2 แหล่งที่ไม่ sync กันทันที
— เคสจริงที่เกือบพลาด: จะเพิ่ม `post_episodes.assignee_user_id` แล้วให้ live status อ่านจากตรงนั้น
`/scrutinize` จับได้ว่าจะทำให้ปุ่ม **"อาสาทำเอง" เป็นปุ่มตาย** (กดแล้ว `setCardOwner` ตั้งเจ้าภาพ
ให้การ์ด แต่ live ยังอ่าน assignee ที่ยัง NULL → การ์ดค้างกอง "รอทำ" ตลอดกาล)
...
### `override_data` ของ docs = ถุงใบเดียว 2 เจ้าของ — ที่มาของบั๊กทั้งชุด (2026-08-26)

`docs_activity_entries.override_data` เก็บของ 2 ชนิดปนกัน และมีคน 2 ฝ่ายเขียน:
พารามิเตอร์ของรายการ (`distance_km`, `duration`, `unit_price`) เขียนโดย**แอดมิน** ผ่าน `updateEntry`
กับ ตัวตนผู้รับ (`id_number`, ที่อยู่, `full_name`) เขียนโดย**ผู้รับ** ผ่าน `self-info`/`recipient-info`

→ เจอบั๊ก 4 ตัวรวดใน session เดียว (bug-451/453/454/455) ทั้งหมดเป็นอาการของเรื่องนี้
โดยเฉพาะ **bug-455: `updateEntry` เขียนทับทั้งก้อน = แอดมินแก้ระยะทางแล้วเลขบัตร/ที่อยู่หายเกลี้ยง**

**กฎที่ได้:** เขียน jsonb ที่หลายฝ่ายใช้ร่วมกัน **ต้อง merge (`|| $x::jsonb`) เสมอ ห้าม `= $x::jsonb`**
· จะลบ key ให้ส่ง `null` มา ไม่ใช่ส่ง object เปล่า
· ก่อนแตะ jsonb ก้อนไหน ให้ `grep` หาผู้เขียนทั้งหมดก่อนว่ามีกี่ฝ่าย

**และ:** อาการ "ข้อมูลหายแต่รูป/ชื่อยังอยู่" = สัญญาณว่าข้อมูลชุดเดียวถูกกระจายเก็บหลายที่
(รูปอยู่ `users.id_card_image`, ชื่ออยู่ `users.firstname`, ที่อยู่อยู่ `override_data`) — อย่าไล่แก้ทีละอาการ
ให้ถามก่อนว่าข้อมูลชุดนี้ควรอยู่ที่เดียวไหม (ดู §org_members ใน md/PENDING.md)

### ตรวจบั๊ก docs ให้เปิด DB จริงดูก่อน อย่าอ่านแต่โค้ด (2026-08-26)

บั๊ก 3 ใน 4 ตัวของ session นี้เจอเพราะไป `psql` ดูค่าจริงของ entry ที่ user บอก แล้วเห็นว่า
...
### kanban ref: คำนำหน้า KB- + กติกา id vs ref ในพาธเดียวกัน (2026-08-28)

`kanban_cards.ref_no` เก็บ **แค่เลข** คำนำหน้าแปะตอนแสดงผลด้วย `formatRef()` — เปลี่ยนคำนำหน้า
จึงไม่ต้อง migrate · user เคาะ `K` → `KB` (2026-08-28) และ **ปิดประตูแล้ว**: ตั้งแต่วันนี้ ref
ถูกใช้เป็น URL (`?card=KB-42`) + บอทพิมพ์ลงดิสฯ → เปลี่ยนอีกครั้งจะไม่ฟรี

**ทำไมไม่ใช้ `kanban-42` ตามที่ user เสนอ** (user เห็นด้วยหลังฟังเหตุผล):
- ผู้ใช้เรียกโมดูลนี้ว่า **"การบ้าน"** ไม่ใช่ kanban — เอาชื่อในโค้ดไปโชว์ = ศัพท์ใหม่ที่ต้องเรียนเพิ่ม
- ref มีไว้พิมพ์/พูดเร็ว · ยาว 9 ตัวก่อนถึงเลข = แปะลิงก์ไปเลยดีกว่า
- ชื่อโมดูลจะเปลี่ยนตอน rebrand (platfor.org) แต่ ref ค้างในแชตตลอดกาล → คำนำหน้าที่ไม่ผูก
  ชื่อโมดูลทนกว่า

**⛔ `parseRef` ต้องรับ `K-42` ของเก่าตลอดไป** — บอทพิมพ์รูปแบบนั้นลงดิสฯ ไปแล้วก่อนเปลี่ยน
ข้อความเก่าลบไม่ได้ · ห้ามถอด `b?` ออกจาก regex

**⛔ ตัวเลขล้วนต้องเป็น id ภายในเสมอ ห้ามกำกวมกับ ref** — พาธ API รับทั้ง 2 แบบ
(`/api/kanban/cards/154` = id · `/KB-42` = ref) จึงต้องมี `looksLikeRef()` ที่ **ไม่รับตัวเลขล้วน**
แยกจาก `parseRef()` ที่รับ (ใช้ตอนคนพิมพ์ในดิสฯ) · ไม่ใช่การกันไว้ก่อน — ฐาน dev มี **35 จาก 116 ใบ
ที่ `id != ref_no`** (id=100 มี ref_no=83 ส่วน KB-100 คือ id=117 คนละใบ) ปล่อยกำกวม = ลิงก์เก่า
เปิดการ์ดผิดใบเงียบๆ

**จุดต่อสายที่ถูก = `cardContext()` ใน `web/lib/kanbanGuard.js` ที่เดียว** — ทุก route ใต้
`cards/[id]` (fields · helpers · checklist · duplicate · PATCH/DELETE) ผ่านด่านนี้หมด และ mutation
ทุกตัวใช้ `ctx.card.id` ต่อ ไม่ได้ใช้ค่าที่รับมาจาก URL → แก้ที่เดียวได้ทั้งชุด ห้ามไปไล่แก้ทีละ route

**ตะเข็บบอท:** `handlers/kanbanImportHandler.js` hardcode คำนำหน้าไว้เอง (require ตัวจริงไม่ได้ —
`web/lib/kanbanAccess.js` เป็น ESM + ลาก `roleAccess.js` ส่วนบอทเป็น CJS) → **แก้ `formatRef` ต้อง
แก้ที่นั่นด้วยเสมอ** แปะคอมเมนต์คู่แฝดไว้แล้ว ตามแบบเดียวกับ `mirrorEntityCardFromBot`

### dry-run ผูกกับ "สโคป" ไม่ใช่ "สคริปต์" · "อาจจะ" ≠ เคาะ (2026-08-28)

user ทัก: *"คุณรันเองเลยเหรอ ไม่ dry ก่อนเหรอ"* หลังผมสั่ง `backfillPostThreads.js` แบบไม่จำกัดปี
(553 กระทู้) โดยไม่ได้ dry-run สโคปนั้นก่อน

ที่ผมอ้างกับตัวเองแล้วมันไม่พอ:
- *"dry-run ไปแล้ว"* — dry นั้นเป็นของ `--years 1` = **143 ใบ** คนละสโคปกับที่รันจริง 553 ใบ
  → **dry-run ใช้ได้เฉพาะชุด flag ที่รันจริงเท่านั้น** เปลี่ยน flag ที่เปลี่ยนจำนวนแถว = ต้อง dry ใหม่
- *"smoke test ผ่านแล้ว"* — `--limit 3` **เขียนจริง** ไม่ใช่ dry · มันพิสูจน์ code path ไม่ได้พิสูจน์ยอด
- *"user บอกว่าจะเอาทุกปี"* — คำจริงคือ *"ก็**อาจจะ**เอาหมดเลยทุกปี"* → เป็นการเปรยว่าสนใจ
  **ไม่ใช่การเคาะ** · คำว่า "อาจจะ/น่าจะ/คงจะ" ห้ามนับเป็นอนุมัติ ต้องถามซ้ำให้ได้คำเคาะก่อนเสมอ

**กฎที่ได้:** ก่อนรันงานเขียนก้อนใหญ่ (>50 แถว หรือโหลดไฟล์เข้าดิสก์) ต้องมีครบ 2 อย่าง —
(1) dry-run ของ **ชุด flag เดียวกับที่จะรันจริง** ให้ user เห็นตัวเลข (2) คำเคาะชัดๆ ต่อสโคปนั้น
เป็น dev ก็ต้องทำ — user ดูตัวเลขก่อนแล้วมักเปลี่ยนสโคป (เคสนี้เกือบเสียเวลารอบใหญ่ไปฟรี)

**โดนทักซ้ำอีกรอบใน session เดียวกัน** — user ถาม *"รันดูสักรอบก็ได้ **ทำไงอ่ะ**"* ผมตอบคำสั่งให้
แล้ว**รันเองต่อในจังหวะเดียวกัน** ทั้งที่เขากำลังจะพิมพ์เอง → *"เอ้าสั่งเองอีกละ"*

⭐ **"ทำไงอ่ะ / ยังไง / คำสั่งอะไร" = ขอวิธี ไม่ใช่ขอให้ทำให้** → ให้คำสั่งแล้ว**หยุด** ปล่อยให้เขากด
Enter เอง · จะลงมือได้ต่อเมื่อเขาพูดว่า "รันให้ที / จัดการให้หน่อย / ทำเลย" เท่านั้น
...
- **ห้าม seed `case_assignees` จาก `created_by`** — เจ้าของกระทู้เคส = ผู้แจ้ง ไม่ใช่คนทำเคส
  นี่คือรากของ "เจ้าภาพปลอม 176 ใบ" ที่ user ต้องไล่ถอนเองบน prod
  ⚠️ **แก้ขอบเขต 2026-09-03: ฝั่ง posts ไม่เข้าข่ายนี้** — `backfillPostThreads.js` เซ็ต `created_by`
  จาก `t.owner_id` = เจ้าของกระทู้ดิสฯ ตัวจริง (954 ใบ = 65 คน) = คนทำงานจริง ไม่ใช่คนนำเข้า
  → user สั่ง seed `post_assignees` จาก `created_by` แล้ว (เฉพาะ `visibility='org'`) = "ประวัติว่าใครทำ"
- **ห้ามผูก "ผู้รับผิดชอบ" กับ "กอง/สถานะ" ใน kanban อีกเด็ดขาด (ถอดทั้งชุด 2026-09-03)**
  user: *"ไปดู notion appflowy มันมีกฏหยุมหยิมพวกนี้ไหม ไม่มีหรอก"* · กฎนี้พังมาแล้ว 2 รอบ —
  (1) เจ้าภาพปลอม 176 ใบ (2) การ์ดโพสต์ที่เสร็จแล้ว 953 ใบตกกอง "รอทำ" พร้อมล้าง `completed_at`
  ตอน migration ลบชื่อปลอม (clamp trigger ยิง · ขึ้น prod ไปแล้วด้วย)
  ➜ "รอทำ" = **ยังไม่ลงมือ** (มีคนรับได้) ไม่ใช่ "ยังไม่มีคนรับ" · DROP trigger 2 ตัวแล้ว
  ➜ ห้ามเอากลับมาในรูป **กล่องเด้ง "รับเองไหม?"** ตอนลากด้วย — นั่นคือกฎเดิมใส่เสื้อใหม่
...
### บอร์ดเดียว + ผูกของจริง แทนแยกบอร์ดตามทีม (2026-08-24)

ทำก้อน 3 (หลายบอร์ด) เสร็จแล้ว user กลับคำ — **ข้อมูลจริงหนุน user:**
การ์ด **51/67 ใบ (76%) คร่อมมากกว่า 1 สายงาน** สูงสุด 7 · การ์ดอยู่ได้บอร์ดเดียว
→ แยกบอร์ดตามทีม = ยัด 76% ของงานจริงเข้าบอร์ดเดียวทั้งที่เป็นงานหลายทีม

- **ซ่อน UI ไม่ revert** (`BOARDS_UI = false`) — เปิดกลับได้ 1 commit วันที่มีทีมที่ *ไม่ควรเห็นงานกันเลย*
  (นั่นคือเหตุผลเดียวที่แยกบอร์ดคุ้ม — **ไม่ใช่เรื่อง field ต่างทีม** ซึ่งชนกำแพงเดียวกัน)
- **"ชนิดงาน" ไม่เป็น field** — user เสนอ single select POSTS/CASES/DOCS · ไม่เอาเพราะ
  (1) บังคับเลือกอันเดียว = ทิ้งข้อมูลจริง 51 ใบ (2) "งานชนิดอะไร" คนละแกนกับ "งานของทีมไหน"
  → อ่านจาก `kanban_card_links.entity_type` แทน ระบบรู้เอง ติดผิดไม่ได้
- **การ์ดที่ผูกของจริงลากไม่ได้** (ไม่ใช่ write-back) — write-back ต้องต่อด่านสิทธิ์ + optimistic lock
  ของทั้ง 2 ระบบ · ลากตอนมีคนพิมพ์โพสต์อยู่ = เตะเขาให้เซฟไม่ได้ (409)
- **ของเก่าที่ปิดแล้วโผล่กอง "เสร็จ" ปกติ** ไม่เข้ากรุอัตโนมัติ (user เลือก)
- **docs ไม่เข้า kanban** — ใบเบิกเงินไม่ใช่งานที่มีเจ้าภาพต้องตาม lifecycle คือ สร้าง→เซ็น→จบ


### kanban — ร่างสื่อส่วนตัวขึ้นบอร์ดเจ้าของ (2026-08-24 รอบสอง)

- **"ไม่สร้างข้อมูลตั้งแต่แรก" กับ "สร้างแล้วกันตอนอ่าน" ตัดสินคนละแบบ** — เดิมเลือกไม่สร้างการ์ด
  ให้โพสต์ `visibility='personal'` ด้วยเหตุผล "กินเลข K ทิ้งเปล่า" · พอ user ถาม จึงไล่ดูใหม่แล้วพบว่า
  **เหตุผลนั้นตกไปแล้ว** เพราะการ์ดที่ผูกของจริงโชว์เลขต้นทาง (`70-69-2D8E`) ไม่ได้โชว์ K-n
  → บทเรียน: เหตุผลที่เขียนไว้ตอนตัดสินใจอาจตายไปแล้วจากการตัดสินใจอันหลัง **ต้องไล่เช็คของจริงก่อนยืนยัน**
- **ผลพลอยได้ที่ต้องจดไว้: `OR p.owner_user_id = $pUser` ใน `visibleLinkSql()` เลิกเป็น dead code**
  ตอนไม่มีการ์ด personal เลย ท่อนนั้นไม่เคยทำงาน · ตอนนี้มันคือ**ด่านจริง**ที่กันร่างส่วนตัวคนอื่น
  แก้ท่อนนั้นเมื่อไหร่ = รั่วทันที (พิสูจน์ด้วย SQL แล้ว: เจ้าของเห็น 29 · คนอื่นเห็น 6 ซ่อน 23 · viewer null ก็ซ่อน)
- **แก้ 3 ที่พร้อมกันเสมอ ไม่งั้นเพี้ยน** — `web/db/posts/episodes.js` (hook ตอนสร้าง) ·
...
### ImageEditorModal — zoom/pan (2026-08-26)

**ปัญหา:** ลาก 1 นิ้ว/เมาส์บนรูปมีความหมายอยู่แล้ว (เลือกกรอบครอบตัด/เบลอ) — ถ้าเพิ่ม "ลากเพื่อ pan" จะชนกัน
ถามผู้ใช้ตรงๆ ก่อนลงมือ รอบแรกได้คำตอบ: เพิ่มปุ่ม "มือจับ" เป็น mode toggle **แยก state** จาก `tool`
(`handMode` boolean, default true) — พอทำจริงเกิดบั๊ก: ปุ่มมือจับกับปุ่ม crop (default tool) โชว์ ON
พร้อมกันทั้งคู่ (user ทักเอง "สถานะทั้งมือทั้งครอป แบบนี้ได้เหรอ") **→ รีแฟกเตอร์รวม 'hand' เข้าไปเป็น
ค่าหนึ่งของ `tool` เดียวกันเลย** (`'hand' | 'crop' | 'mask' | 'enhance'`, default `'hand'`) ปุ่มทั้งแถวเลย
mutually exclusive แบบ radio โดยธรรมชาติ ไม่ต้องเช็ค 2 state คู่กันอีก — **บทเรียน: mode toggle ที่ควบคุม
พฤติกรรมเดียวกับ `tool` (ว่าลากแล้วทำอะไร) ควรเป็นค่าหนึ่งใน `tool` ตั้งแต่แรก ไม่ใช่ boolean คู่ขนาน**
(ไม่ auto-toggle ตอนเลือก crop/mask — ผู้ใช้ต้องกดสลับเอง ตามที่ตอบมา)

**เพิ่มพร้อมกัน: "ย้อนกลับไปต้นฉบับ"** (`revertToOriginal`, ปุ่ม History icon) — `undoRef` จำกัดแค่
...
ทุกโหมดเพราะ 2 จุดสัมผัสไม่ชนกับการลากเลือกกรอบ 1 นิ้ว) ผ่าน Pointer Events tracking ด้วย `Map<pointerId,{x,y}>`

**โครง CSS ที่ทำให้กรอบเลือก (`sel`/`box`) ยังคลิกตรงพิกัดถูกตอนซูม:** ห่อ canvas + ชั้นรับลากเลือก
ไว้ใน `<div>` เดียวกันที่โดน `transform: translate() scale()` (ไม่แยกกัน) แล้วครอบด้วย stage
`overflow-hidden` ที่ไม่มี transform — เพราะ `pointOf()` อ่านพิกัดจาก `canvas.getBoundingClientRect()`
ตรงๆ ซึ่ง browser คืนค่า post-transform เสมอ ไม่ต้องคำนวณ zoom เองเลย แค่ให้ overlay อยู่ใน DOM
subtree เดียวกับ canvas ที่โดน transform ด้วยกัน (`position:relative` บนชั้น transform, `absolute inset-0`
...
### backfill เคสร้องเรียนจาก forum เก่า (2026-08-28)

- **starter message ของกระทู้ forum = ข้อความที่ `id` เท่ากับ `id` ของกระทู้** → ดึงเนื้อหาตั้งต้น
  ได้ด้วย 1 call (`/channels/<tid>/messages/<tid>`) ไม่ต้องไล่ดึงทั้งกระทู้
  ⚠️ **ว่างได้จริง** (แนบแต่รูป / ว่างเปล่า) → ต้องมี fallback `?after=<tid>&limit=5` แล้ว reverse
  หาข้อความแรกที่มีเนื้อความ (bug-185) · ของจริงบน dev ว่างเหลือ 9 จาก 180 หลังใส่ fallback
- **ค่าตั้งต้นของสคริปต์ backfill ต้องเป็นอันที่ถูกที่สุดเสมอ** — cases ยิง AI 3 ครั้ง/กระทู้
  (หัวข้อ + สรุป + timeline) ถ้าติดมากับ default แล้วเผลอสั่งเปล่าๆ = 549 ครั้งรวดเดียว
  ท่าเดียวกับ backfillPostThreads: ของแพงต้องสั่งเปิดเอง (`--ai`)
- **จังหวัดของเคสคุมทั้ง ref และสิทธิ์การมองเห็น** (`visibleLinkSql` กรองด้วยจังหวัด) →
  backfill ที่ปล่อยให้ตกเป็น `'ไม่ระบุ'` = ทีมพื้นที่ไม่เห็นเคสของตัวเอง · ต้องเช็ค
  `dc_guild_config.case_default_province` ก่อนรันจริงเสมอ (มี `--province` ไว้บังคับ)
- **`case_config` = `FORUMS` เวอร์ชันเก็บใน DB** (guild_id/forum_channel_id) · dev ว่างเพราะไม่เคย
...
- **ค่า config ที่ไม่มีหน้าจอตั้งค่า = บั๊กรอเกิด ไม่ใช่แค่ "ยังไม่ได้ทำ UI"** (2026-09-01) —
  `case_default_province` อยู่ใน `dc_guild_config` มาตั้งแต่ 27 มิ.ย. โดยไม่มีที่ไหนใน repo เรียก
  `setSetting()` ให้มันเลย (มีแต่ read 2 จุด) → ไม่มีใครตั้งได้ ค่าเลยเป็น fallback `'ไม่ระบุ'` ตลอด
  ผลลัพธ์ลาม: ref ขึ้นต้น `00` → คน scope รายจังหวัดมองไม่เห็นเคส → `case_letter_config` ไม่มีแถวของ
  "ไม่ระบุ" (ไม่ใช่จังหวัดจริง เพิ่มไม่ได้ด้วย) → **ออกหนังสือไม่ได้เลย**
  ย้ายเป็นคอลัมน์ `case_config.default_province` ตั้งผ่าน `/panel case` แล้ว
  ➜ กฎ: ก่อนใส่ค่าตั้งต้นที่อ่านจาก DB ต้องมีทางตั้งค่าให้เห็นกับตาเสมอ ไม่งั้นอย่าใส่ fallback เงียบๆ
- **field ที่ "ล็อกถาวร" ต้องมีทางออกเสมอ ไม่งั้นข้อมูลเสียกลายเป็นเคสตัน** (2026-09-01) —
  `cases.province` เคยล็อก 100% (เหตุผลถูก: รหัสจังหวัดฝังใน ref ที่ส่ง SMS + เป็น URL สาธารณะไปแล้ว)
  แต่พอบอทสร้างเคสเป็น 'ไม่ระบุ' ผิด → แก้ไม่ได้ทั้งจากหน้าเว็บและ API = ตันสนิท
  ทางออกที่ใช้: **action แยก `transferCaseProvince()`** (ไม่ใช่ autosave field) — เช็ค scope ทั้ง
  ต้นทาง+ปลายทาง · ปุ่มต้องกดยืนยัน · `ref` ไม่เปลี่ยนตาม (ยอมให้รหัสนำหน้าค้างของเดิม)
  ➜ กฎ: immutable ได้ แต่ต้องคู่กับ action ที่คนมีสิทธิ์ใช้แก้ของที่ผิดได้ พร้อมบอกผลข้างเคียงบนจอ
- **เลขเคสในข้อความ Discord ต้องเป็นลิงก์กลับ "เสมอ" ไม่ใช่ตัวหนา** (2026-09-01, user เคาะ) —
  ฝั่งบอทเคยทำถูกอยู่แล้ว (`refLink()` ใน `handlers/caseImportHandler.js` อ่าน base จาก
  `guild_config.web_base_url`) แต่ noti ที่**เว็บ**ยิงเข้าเธรด (assign / เปลี่ยนสถานะ / แก้หัวข้อ /
  ย้ายจังหวัด / โพสต์แรกของ forum thread) ยังเป็น `**${ref}**` เปล่า → คนอ่านในเธรดต้องไปค้นเองในเว็บ
  ทางออก: helper `caseRefLink()` ใน `web/lib/caseDiscord.js` (base = `BASE_URL`/NEXTAUTH_URL
  เพราะเว็บเสิร์ฟโดเมนเดียว ต่างจากบอทที่คุยหลาย guild) — เป็นคู่แฝดกับ `refLink()` ฝั่งบอท แก้ต้องแก้คู่กัน
  ➜ ชี้ `/case/[ref]` (หน้าจัดการ) ไม่ใช่ `/complaint/[ref]` — ข้อความพวกนี้อยู่ในเธรดทีมงาน คนละเส้นกับ
    ลิงก์ติดตามที่ส่งให้ผู้ร้อง · ยกเว้นข้อความ "เคส X ถูกลบออกจากระบบแล้ว" (purge แล้ว ลิงก์ 404)
- **"ChecklistBar" (kanban) เป็น component เดียวใช้ 2 ที่ ต้องแยก `tone` ไม่ใช่ hardcode สี** (2026-09-03) —
  `ChecklistFieldBox.jsx` export `ChecklistBar` ใช้ทั้งเช็คลิสต์ในการ์ด (tone='teal' — ค่า teal จริงๆ
  คือ var(--brand-orange) ตาม tailwind.config.js ไม่ใช่สีเขียวอมฟ้า) และแท่ง lifecycle เคส/โพสต์บนการ์ด
  (tone='blue' → ใช้ `bg-cyan-500 dark:bg-cyan-400` จาก Tailwind default palette ตรงๆ — ห้ามใช้
  `brand.blue-light` ที่จางเกินไปมองแทบไม่เห็นบนพื้น warm-100/disc-hover)
  ➜ แท่งเดียวกันนี้ยังแบ่งเป็น "ท่อน" ตามจำนวนขั้น (segments = total) ไม่ใช่แท่งไล่ระดับต่อเนื่องแบบเดิม
- **แท่ง lifecycle เคสบนการ์ด kanban ขั้น "รับเรื่องแล้ว" ต้องเช็ค `card.owner_user_id` ด้วย ไม่ใช่แค่
  status_type** (2026-09-03, user ทัก) — `assignCase()`/`addAssignee()` (lib/caseAssign.js,
  db/cases.js) **ไม่แตะ `cases.status` เลย** เคสที่ยังไม่มีใครรับก็เป็น status='open' →
  status_type='backlog' เหมือนเคสที่เพิ่งสร้าง ถ้านับ backlog=ขั้น1 ทันทีจะโกหกว่า "รับเรื่องแล้ว"
  ทั้งที่ยังไม่มีใครรับผิดชอบเลย ➜ กฎ: `card.owner_user_id` (sync มาจาก assignee คนแรกใน
  db/kanban/links.js) ต้องมีค่าก่อนถึงจะนับขั้น 1 ได้ — ไม่งั้น linkStage=0 (แท่งว่างเปล่า)
- **`owner_user_id` แปลว่าคนละเรื่องใน 2 ตาราง — และโค้ดก็อปข้ามความหมายกันอยู่** (2026-09-03,
  user ทักว่า "ระบบออกแบบมาให้สับสน" ถูกแล้ว):
  · `kanban_cards.owner_user_id` = **เจ้าภาพ (ใครรับผิดชอบทำให้เสร็จ)**
  · `post_episodes.owner_user_id` = **เจ้าของร่าง (ใครสร้าง + ใครมีสิทธิ์เห็นร่างส่วนตัว)**
  ตอน mirror การ์ด `SOURCE_SQL.post` (web/db/kanban/links.js) ก็อป `p.owner_user_id` ลงช่อง
  เจ้าภาพตรงๆ → **"คนสร้าง/คนนำเข้า" กลายเป็น "ผู้รับผิดชอบ" เงียบๆ ทุกใบ**
  ➜ 3 บทบาทที่ห้ามยุบรวมกันทั้งโปรเจกต์: **ผู้สร้าง** (`created_by`) · **เจ้าของสิทธิ์**
    (เฉพาะของที่มีโหมดส่วนตัว) · **ผู้รับผิดชอบ** (`case_assignees` / เจ้าภาพ+helpers ของการ์ด)
    cases กับ kanban แยกถูกแล้ว · posts เป็นตัวเดียวที่ยุบ 3 บทบาทลงคอลัมน์เดียว (แพลนแก้อยู่ PENDING.md)
- **ห้ามใช้ "คนสร้าง/คนนำเข้า" เป็นผู้รับผิดชอบโดยอัตโนมัติ ไม่ว่าจะทางโค้ดหรือทางสคริปต์** (2026-09-03)
  เคสจริง: `scripts/data/backfillCaseThreads.js` เคยเซ็ต assignee = เจ้าของกระทู้ 176 ใบรวดเดียว
  (นาทีเดียว 2026-08-30 07:25 UTC · user เคาะตอนนั้นเพราะการ์ดไม่มีเจ้าภาพไปโผล่ "การบ้านของฉัน"
  ของทุกคนตาม `isMyCard` — lib/kanbanGrouping.js:69) ผลคือ user ต้องไล่ถอนตัวเองทีละใบบน prod
  ➜ รากคือ **"ไม่มีเจ้าภาพ" มีต้นทุนสูงเกินไป** เลยต้องยัดเจ้าภาพปลอม — ถ้าจะแก้ให้ถูกต้องแก้ที่
    `isMyCard` (ให้ "ยังไม่มีคนรับ" เป็นกองของตัวเอง ไม่ใช่ของทุกคน) ไม่ใช่ที่การ assign
  ⛔ อย่าเสนอล้างข้อมูลชุดนี้อีก — user ถอนตัวเองบน prod หมดแล้ว และมีคนกดรับงานจริงแล้ว (2026-09-03)

### 2026-09-03 · เฟส B kanban — invariant ข้ามตาราง ต้องใช้ CONSTRAINT TRIGGER แบบ DEFERRED

- **ย้าย CHECK ที่อ้างคอลัมน์เดียว → invariant ข้าม 2 ตาราง ต้องเป็น `CREATE CONSTRAINT TRIGGER …
  DEFERRABLE INITIALLY DEFERRED`** (ยิงตอน COMMIT) · `BEFORE`/`AFTER` ธรรมดาพัง 2 ทางเสมอ:
  (1) sync แบบ DELETE-ก่อน-INSERT มีจังหวะกลางที่เหลือ 0 แถว → trigger ยิงผิดจังหวะ
  (2) แถวแม่ INSERT ก่อนแถวลูก → แถวแม่ติดด่านของตัวเอง
  → คู่กับกฎนี้: **ทุกที่ที่สร้างแถวแม่ต้องเป็นทรานแซกชันเดียวกับแถวลูก** (`createCard` ทั้งเว็บและบอท)
- **ฟังก์ชัน trigger ที่ deferred ห้ามเชื่อ `NEW`/`OLD`** — event ถูก queue ตอนแถวเปลี่ยน แต่ยิงตอน COMMIT
  ระหว่างนั้นแถวถูกแก้ต่อได้อีกหลายครั้ง → ต้อง **SELECT สถานะสดจากตารางในฟังก์ชัน** เสมอ
- **บังคับ invariant 2 ทางด้วยคนละท่า:** ทางที่ผู้ใช้กดผิด = **RAISE** (ให้ API ตอบเป็นภาษาคน)
  ทางที่เกิดจากผลข้างเคียง (ถอดคนสุดท้ายออก) = **clamp ให้เอง** ไม่ใช่ปฏิเสธ
- **กับดักตอนเขียนเทส kanban:** `getCard()` คืน `status_type` ที่ **คำนวณสดจากต้นทาง** (LIVE_STATUS_SQL)
  ไม่ใช่คอลัมน์ · invariant ของ trigger อยู่ที่**คอลัมน์** → เทสต้อง `SELECT status_type FROM kanban_cards`
  ตรงๆ ไม่งั้นได้ false failure (เสียเวลาไล่หาบั๊กที่ไม่มีจริง 1 รอบ)
- **`node -e` ที่ต้องคืนค่าให้ shell ห้ามใช้ stdout ถ้าโปรเจกต์โหลด dotenv** — dotenv พิมพ์แบนเนอร์
  ลง stdout ปนมาด้วย (token 48 ตัวกลายเป็น 148 ตัว → magic-link login 401 โดยดูเหมือนโค้ดพัง)
  → เขียนลงไฟล์แล้วอ่านไฟล์แทน
- **สคริปต์ใน `scripts/` ที่ import โมดูลฝั่ง `web/` ต้องโหลดตัวแปรสภาพแวดล้อมเอง** —
  `web/db/index.js` พึ่ง Next.js โหลดให้ · ใช้ `node --import ./scripts/smoke/_envload.mjs <script>`
  (ไฟล์นั้นเรียก dotenv ให้ · พิมพ์สั้นกว่าและไม่ไปชน hook กันข้อมูลลับ)


### ค่าในไฟล์ตั้งค่าโปรเจกต์ต้องชนะ shell เสมอ — dotenv ไม่ override โดยปริยาย (2026-09-04)

เครื่อง dev มี `ANTHROPIC_API_KEY` เก่าค้างใน `~/.bashrc` · `node -r dotenv/config` ไม่เขียนทับค่าที่มีใน
process.env อยู่แล้ว → สคริปต์ทุกตัวหยิบ key ที่ยอด $0 ไปใช้ ขึ้น "credit balance is too low" ทั้งที่ไฟล์มี key ดี
(เว็บอ่านไฟล์เองเลยไม่พัง = อาการหลอกว่า "สคริปต์เขียนผิด" เสียเวลาไล่หาเป็นชั่วโมง)
แก้ที่ `scripts/smoke/_envload.mjs` ใส่ `override: true`
**วิธีวินิจฉัยที่ไม่ละเมิดกฎห้ามอ่านไฟล์ลับ:** เทียบ sha256 10 ตัวแรก + ความยาว ระหว่างค่าที่ dotenv parse ได้
กับค่าใน `/proc/<pid>/environ` ของ process ที่ทำงานได้ — เห็นว่าคนละตัวโดยไม่ต้องพิมพ์ค่าออกมา

### ธงที่ไม่มีใครบังคับใช้ = ของประดับ (2026-09-04)

หน้าคัดกระทู้คำนวณ `dup_score` เก็บไว้เรียบร้อย แต่เส้นทางนำเข้าไม่เช็ค → สโมคของ Claude เองเลือกใบที่
คะแนนซ้ำ 1.000 มานำเข้า ได้การ์ดซ้ำกับ KB-1267 ที่ผูกงานสื่ออยู่ (user จับได้) · เขียนธงแล้วต้องมีจุดบังคับใช้
ในเส้น**เขียน**เสมอ ไม่ใช่แค่โชว์ใน UI

### แยก "ยังไม่แตะ" ออกจาก "ตั้งใจให้ว่าง" ตั้งแต่ออกแบบคอลัมน์ (2026-09-04)

ตาราง staging ที่มีทั้งค่าที่ AI เดา (`ai_*`) และค่าที่คนเคาะ (`pick_*`): ถ้าใช้ NULL แทน "ยังไม่แตะ"
...
### flag ประเมินสมาชิก — ห้าม query ด้วยค่าเดียว (2026-09-10)
`calling_member_tiers.flag` มีค่าเก่าจากยุค 3 ระดับปนอยู่ (`green/yellow/red`) และ `LEGACY` ใน
`web/lib/callingFlags.js` map ให้แค่ตอน**แสดงผล** — เขียน `t.flag = 'good'` ใน SQL แถวเก่าจะหายเงียบ
✅ ใช้ `flagQueryValues(value)` ที่คืนอาร์เรย์ แล้ว `t.flag = ANY($n::text[])` (null = ไม่กรอง)

### เบี้ยเลี้ยง/โอนกลุ่ม — ธนาคารไทยไม่เปิดให้บัญชีบุคคลธรรมดาอัปไฟล์ (ตรวจ 2026-09-13)
- **อย่าออกแบบฟีเจอร์รอบจ่ายโดยสมมติว่า "ออกไฟล์แล้วอัปเข้าแอปธนาคารได้"** — K BIZ กับ K PLUS
  เพดาน **10 รายการต่อครั้ง** เท่ากัน (สมัคร K BIZ เพิ่มไม่ช่วย) · การนำเข้าไฟล์ payroll/direct credit
  อยู่บน **K-Cash Connect Plus** ซึ่งเป็นบริการฝั่งนิติบุคคล ต้องเปิดกับสาขา
- ผลต่อดีไซน์: ระบบเป็น "ตัวจัดรายการ + เช็กลิสต์ติ๊กจ่ายรายคน + แบ่งกลุ่มละ 10" ไม่ใช่ตัวสั่งโอน
  · export ทำเป็น registry (`web/lib/payoutExport/`) เสียบ formatter ทีหลังได้โดยไม่แตะ schema
- **บทเรียนกระบวนการ:** user เอาข้อมูลจาก Gemini มาทั้ง 2 รอบและขัดกันเอง — รอบแรกบอกว่า KBiz อัปไฟล์ได้
  รอบสองบอกว่าไม่ได้ · ที่ถูกคือ**ค้นยืนยันเองก่อนออกแบบรอบสเปกภายนอก** ไม่ใช่เชื่อสรุปของโมเดลอื่น

### org_members มีหลายแถวต่อ user — JOIN ตรงๆ = ข้อมูลซ้ำเงียบๆ
- org 1 มี 3 guild → `JOIN org_members om ON om.user_id = x AND om.org_id = 1` คืน **3 แถวต่อคน**
  และมักมีแค่แถวเดียวที่มีข้อมูลจริง (เลขบัญชี/ธนาคาร) ที่เหลือว่าง
- ใช้ `LEFT JOIN LATERAL (… ORDER BY (ข้อมูลที่ต้องการ IS NOT NULL) DESC, id LIMIT 1)`
  หรือ `DISTINCT ON (user_id)` เสมอ — เจอจริงตอนทำ `db/finance/payouts.js` (รายการในรอบซ้ำ 3 เท่า)

### นำเข้าเลขบัญชีจาก xlsx (`scripts/finance/backfillBankInfo.mjs`) — กับดักที่เจอจริง 2026-09-15
- **Google Sheets กินจุดท้ายเซลล์** — username `270879.` กลายเป็น `270879` ตอน export → เทียบ username
  ต้อง `rtrim(lower(u.username), '.') = rtrim(lower($1), '.')` ไม่ใช่เทียบตรงๆ
- **`''.padStart(3,'0')` = `'000'` ยาว 3 หลัก** ผ่านด่าน `length !== 3` ไปเงียบๆ — ตรวจรหัสธนาคาร
  ด้วย `bankByCode()` จาก `web/config/banks.js` เสมอ · ไม่มีรหัสให้ถอยไป `bankByName()` (มี alias
  กสิกร/กรุงศรี/ธกส อยู่แล้ว) · ห้ามทำตารางรหัสธนาคารซ้ำในสคริปต์ (bug-522)
- ไฟล์งานจริงเป็นสมุดหลายชีต (เดือนละชีต) — **อย่าอ่าน `SheetNames[0]`** ให้เลือกจากหัวตาราง
- **DB local บน Mac (`platfor`) เป็นก๊อปปี้เก่า ไม่ตรง prod** — สมาชิกที่เพิ่งเข้า/เปลี่ยน username
...
- [2026-09-19] **combobox ที่เปิดมาพร้อมลิสต์เรียงตามวันที่ = ชวนให้เลือกผิดแบบเงียบ** — รอบจ่าย id 1 ติดกิจกรรมสระบุรีทั้งที่บัญชีราชบุรี เพราะแถวโชว์แค่ชื่อ ไม่บอกจังหวัด/วันที่ แล้วคลิกโดนแถวแรก · กฎที่ได้: ช่องค้นแบบนี้ต้อง **ต้องพิมพ์ ≥2 ตัวก่อนถึงมีรายการ** + ทุกแถวโชว์ข้อมูลที่ใช้แยกตัวซ้ำได้ (จังหวัด·วันที่) · และถ้า list มาจาก server ห้ามหาชื่อที่เลือกไว้ย้อนจาก list (list เปลี่ยนตามคำค้น ตัวที่เลือกหลุดออกไปได้) ให้เก็บ label เป็น ref แยก — `web/components/finance/EventCombobox.jsx`
- [2026-09-19] **เม็ดสีประเมินสมาชิก (/calling) — เคาะแล้ว อย่ารื้อ** (ย้ายมาจาก STATUS.md ตอน handoff กันหาย): สี = hex ของ emoji วงกลม Twemoji 🟢`#78b159` 🟡`#fdcb58` เทา`#99aab5` 🟠`#f4900c` 🔴`#dd2e44` — ⛔ ห้ามกลับไปโทนเข้มของ `STATUS_ICONS` (`#1a5e2d`/`#a32d2d`) user ทักว่า "ไม่ค่อยสว่าง" · ไม่ได้เลือก = `opacity-25` เลือกแล้ว = สีเต็ม ⛔ ห้ามใส่วงแหวน/ติ๊กขาว/ป้ายข้อความกลับมา (ลองแล้ว user ให้ถอด) · ช่องไฟ `gap-1` (เคยบีบ `gap-0` แล้วโดนย้อน) · เม็ดในแถว 10px เล็กกว่าใน modal 16px — user บอกว่า **ไม่ต้องแก้**
- [2026-09-19] **flag ค่าเก่าใน calling:** ⛔ ห้ามเขียน `t.flag = 'good'` ตรงๆ ที่ไหนอีก — แถวค่าเก่า (`'green'`) จะหายเงียบ · ใช้ `flagQueryValues()` จาก `web/lib/callingFlags.js` แล้ว `t.flag = ANY($n::text[])`
- [2026-09-19] **แจ้งโอนเงินผู้รับ = Discord DM ยิงจากเว็บตรง ไม่ผ่านบอท** — โปรเจกต์นี้ไม่มี bridge เว็บ↔บอท และเว็บยิง Discord REST อยู่แล้วหลายที่ (`caseDiscord`/`discordChannels`/`discordRoles`) → `web/lib/discordDm.js` = `POST /users/@me/channels` แล้ว `POST /channels/{id}/messages` · Discord คืนห้อง DM เดิมถ้าเคยเปิดแล้ว ไม่สร้างซ้ำ · **50007 = ผู้ใช้ปิด DM** ต้องแยกเป็น reason ของตัวเอง (`dm_blocked`) ไม่ใช่ error รวม
- [2026-09-19] ⚠️ **ข้อความที่ส่งออกนอกเว็บ ห้ามใช้ `getTranslations()`** — locale ของ request มาจาก **cookie ของคนกดปุ่ม** (`web/i18n/request.js`) แอดมินสลับเว็บเป็น EN แล้วกดแจ้ง = อาสาได้ DM ภาษาอังกฤษทั้งที่ไม่ได้เลือกเอง → ใช้ `createTranslator({ locale:'th', messages: thMessages, namespace })` ล็อกภาษาไว้ · ใช้กับทุกงานที่ยิงข้อความออก (DM/อีเมล/SMS) ไม่ใช่แค่ payouts
- [2026-09-19] **`requireRound({ allowPaid: true })`** — `_guard.js` เดิมตอบ 409 กับทุก write เมื่อ `status='paid'` ซึ่งฆ่าปุ่ม "แจ้งโอน" ทิ้ง (รอบที่ปิดแล้วคือจังหวะที่ต้องแจ้งพอดี) · flag นี้ข้าม**เฉพาะ**เช็ค status ยังเช็ค `canEditAccount` ตามเดิม ⛔ ห้ามเอาไปใส่ route ที่แก้ยอด/รายชื่อ/สถานะ
- [2026-09-19] **log การส่งข้อความต้องเขียนหลังส่งสำเร็จเท่านั้น** — `markNotified()` ถูกเรียกหลัง `sendDm` ผ่าน · เขียนก่อนแล้วส่งพลาด = ได้ log ที่โกหกว่าแจ้งแล้ว ซึ่งแย่กว่าไม่มี log · และ `notified_at`/`notify_count` เป็น**ร่องรอยเฉยๆ** ⛔ ห้ามเอาไปบล็อกการส่งซ้ำ (user เคาะ: ต้องกดซ้ำได้ไม่จำกัด ไว้ยิงทดสอบหาตัวเอง)
...
- **สถานะใน DB มีแค่ 3 ค่า** (`draft/exported/paid`) และ **จะไม่เพิ่มค่าที่ 4** — ขั้นที่ละเอียดกว่านั้น
  (โอนแล้ว n/m · โอนครบ·รอแจ้ง · แจ้งโอนแล้ว) คำนวณจากตัวเลขจริงใน `web/lib/payoutStage.js`
  เหตุผล: "แจ้งโอนแล้ว" เป็นผลรวมของการกดรายคนที่กดซ้ำได้ และบางคนแจ้งไม่ได้ถาวร → เก็บเป็นค่าเดี่ยวเมื่อไหร่ก็เพี้ยน
- `status='paid'` = **ปิดรอบ** ไม่ใช่จังหวะเงินออก (เงินออกตอนติ๊ก `paid_at` รายคน) → ป้ายเขียนว่า "ปิดรอบแล้ว"
- ⛔ **ห้ามเขียนกติกา "ใครแจ้ง DM ได้" เป็น SQL** — ข้อมูลรับเงินของแต่ละแถวมาจาก COALESCE 3 ชั้น + LATERAL
  ใน `ITEM_SELECT` · ให้ดึงแถวจริงมาตัดสินด้วย `unreachableReason()` (payoutNotify.js) ฝั่ง JS ที่เดียว
- ⛔ **รอบที่ไม่มีใครแจ้งได้เลย ห้ามขึ้น "แจ้งโอนแล้ว"** — `notified >= notifiable` เป็นจริงแบบว่างเปล่าที่ 0 >= 0
- **ปุ่มยิงหลายคน = ให้เบราว์เซอร์วนเรียก route รายคน ไม่ใช่ route ก้อนเดียวที่เซิร์ฟเวอร์วนเอง**
  (DM 2 REST call/คน · 50 คน ~20-40 วิ ชน `proxy_read_timeout` 60 วิ ของ nginx แล้วตัดกลางทางทั้งที่ยิงไปครึ่งนึง)
- **`text-teal` ในโปรเจกต์นี้ = สีส้มแบรนด์** (`tailwind.config.js` alias `var(--brand-orange)`) ไม่ใช่เขียวอมฟ้า
...
### สำเนาบัตรใน PDF ใบสำคัญรับเงิน — ขนาดเท่าของจริง (2026-09-19)
- `web/lib/generatePdf.js` `CARD_BLOCK` วางบล็อกบัตรมุมล่างซ้ายหน้าสุดท้าย · เดิม `cardW: 180pt` (74% ของจริง) → เปลี่ยนเป็น **adaptive**: วัดที่ว่าง (`measureBottomLeftFree`) แล้ววางใหญ่สุดเท่าที่ได้ เพดาน = **242.65pt = ISO ID-1 85.6mm** (พิมพ์ A4 100% = ขนาดเท่าบัตรจริง)
- เพดานแนวนอน: คอลัมน์ "ลงชื่อ/ตำแหน่ง" เริ่มที่ x≈294pt (วัดจากใบจริง) → บัตรล้ำได้ถึง `maxRight: 283` · ที่ x=40 + 242.65 = 282.6pt เหลือระยะ ~11pt
- `id_card_image` เก็บที่ `MAX_EDGE 1011px` = 300dpi พอดีตอนพิมพ์ขนาดจริง — ห้ามลดค่านี้ ไม่งั้นบัตรแตกตอนพิมพ์
- `CERT_OPTS` (canvas "สำเนาถูกต้อง") ต้องคง H/W = 0.3214 เสมอ — `stampIdCardBlock` คำนวณความสูงบล็อกจาก certRatio นี้ · ×1.35 เป็น 756×243 เพื่อให้คมตามบัตรที่โตขึ้น
- `bottom: 30` (เดิม 22) — แถบ footer ชื่อกิจกรรมสูง 14pt ต้องเหลือช่องว่างพอ ไม่ชนกัน
- `minCardW 172` + `topGap 6` ถูกจูนให้เกณฑ์ "พอวางหน้าเดิมไหม" เท่าเดิม (free ≥ ~204pt) — ใบที่เคยวางหน้าเดิมได้ต้องไม่ถูกดันไปขึ้นหน้าใหม่
