...
## 🧹 กวาด `md/PENDING.md` ครั้งใหญ่ (เสร็จ · ยังไม่ commit)

user ทักว่า session ก่อนตอบผิด ("pending ไม่มีอะไรต้องเคลียร์" ทั้งที่ยาว 2,527 บรรทัด) — **ตรวจแล้ว user ถูก**

**หลักฐานที่ทำให้ของเก่าตายยกแผง:** `prod HEAD = origin/master = d2e5980e` (ตรวจผ่าน ssh 2026-09-19) ·
587 commit ตั้งแต่ 1 ส.ค. · ไม่มีโค้ดค้างใน working tree นอกจากงานวันนี้ ·
marker `production ทำถึงตรงนี้` อยู่บรรทัด 1959/1964 ของ `scripts/migration/migration.sql`
→ ป้าย "เสร็จ local · ยังไม่ push/deploy" ของ ก.ค.–ส.ค. **ตายหมดแล้ว** และบล็อก SQL ท้ายไฟล์นั้นรันครบแล้ว

**ผล:** `md/PENDING.md` 2,527 → 1,697 บรรทัด (88 → 75 หัวข้อ)
- ย้ายประวัติออก 28 หัวข้อ → `md/archive/DONE-2026-07.md` (ใหม่) · `DONE-2026-08.md` (ใหม่ 831 บรรทัด) · `DONE-2026-09.md` (ต่อท้าย)
- รื้อหัวข้อ POSTS 292 บรรทัดที่พาดหัวว่า "ยังไม่เขียนโค้ดสักบรรทัด" (ทั้งที่ `/posts` ใช้งานจริง) → เหลือ 24 บรรทัด
- ยุบของซ้ำเป็น 3 กองรวม: **§👁️ ขึ้น prod แล้ว เหลือยังไม่มีใครกดดูด้วยตา** (22 ข้อ + 5 สคริปต์ที่ต้องยืนยัน) ·
  **§🌍 หนี้ i18n** · **§⛔ กฎที่ห้ามกลับคำ** (12 ข้อ สกัดจากหัวข้อที่ย้ายออก — ห้ามลบ)
- ลบ 2 หัวข้อที่ไม่เหลืองานค้างสักข้อ · ย้าย 2 ไอเดียที่ user ตอบ "ไม่ชัวร์" ไปกอง **§💭 ไอเดียพับไว้** ท้ายไฟล์
  (🎮 engagement Discord · 🌩️ console.ppleth.ai) — **ไม่ได้ทิ้ง รอเคาะรอบหน้า** · Rebrand user สั่งให้อยู่ที่เดิม
- ตรวจแล้วไม่มีบรรทัดไหนหาย (เทียบ 1,886 บรรทัดเนื้อหาเดิมกับไฟล์ใหม่ทั้งชุด) · แก้ความลึกลิงก์สัมพัทธ์ใน archive ให้ครบ
- backup ไฟล์เดิมอยู่ที่ scratchpad ของ session

**ที่ยังไม่ได้ทำ:** ยังไม่ commit · หัวข้อ "🔜 จัดบ้าน `md/`" (93 บรรทัด) ยังค้างอยู่ — งานนั้นคือย้ายไฟล์เข้าโฟลเดอร์ คนละเรื่องกับรอบนี้

⚠️ **STATUS ข้างล่างนี้ค้าง** — เขียนว่า "ยังไม่ commit" แต่งาน finance/payouts ขึ้น prod ไปแล้ว (`e40a8c49` · `d08408a8` · `d2e5980e`)
