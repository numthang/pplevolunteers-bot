...
// discord ไม่อยู่ใน list — link ได้แต่ยัง unlink ไม่ได้ในเฟสนี้ (discord_id เป็น key ของฟีเจอร์เยอะ)
...
export async function DELETE(req) {
  ...
  if (provider === 'passkey') {
    const identities = await getUserIdentitiesByUser(session.user.userId)
  ...
  await unlinkIdentityByUser(session.user.userId, provider, provider_id || null)
