...
// PROTEIN_ENUM ดึงจาก wiki (cooking_ingredients grp='protein') สดทุก request — ดู pattern เดียวกับ gates-suggest/route.js
...
async function getProteinEnum() {
  try {
    ...
    console.error('[cooking/import] protein enum query failed:', err)
    return PROTEIN_ENUM_FALLBACK
  }
}

function buildSystem(proteinEnum) {
  return `คุณเป็นผู้ช่วยสร้างข้อมูลเมนูอาหารไทย/ทั่วไปแบบ simple จากชื่อเมนูที่ผู้ใช้ให้มา
...
รูปแบบ JSON (ทุก field ต้องมี):
{
  ...
  "food_groups": ["protein","veg"],        // เลือกจาก: ${FOOD_GROUP_ENUM.join(', ')}
  ...
  "gates": {
    "protein": ["chicken"],                   // ⚠️สำคัญ: โปรตีนที่เป็นตัวตัดสินว่าทำได้ไหม — enum: ${proteinEnum.join(', ')}
...
- gates.protein ต้องเป็น subset ของ enum เท่านั้น: ${proteinEnum.join(', ')} — ถ้าเมนูใช้หมูสับให้ใส่ "pork" ไม่ใช่ "หมู"
...
export async function POST(req) {
  ...
  menu.gates.protein = menu.gates.protein.filter((p) => proteinEnum.includes(p))
