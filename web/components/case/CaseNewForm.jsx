...
export default function CaseNewForm({ fixedProvince, provinces, categories }) {
  ...
  function onPickFiles(e) {
    ...
    if (picked.length > MAX_FILES) { alert(`แนบได้ไม่เกิน ${MAX_FILES} ไฟล์`); e.target.value = ''; return }
    const tooBig = picked.find(f => f.size > MAX_SIZE)
    if (tooBig) { alert(`ไฟล์ "${tooBig.name}" เกิน 10MB`); e.target.value = ''; return }
    setFiles(picked)
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!title.trim()) { alert('กรุณาใส่หัวข้อเรื่อง'); return }
    if (!detail.trim()) { alert('กรุณาใส่รายละเอียด'); return }
    if (!name.trim()) { alert('กรุณาใส่ชื่อ'); return }
    if (!phone.trim()) { alert('กรุณาใส่เบอร์โทรศัพท์'); return }
    if (!consent) { alert('กรุณายินยอมให้เก็บข้อมูลก่อนส่งเรื่อง'); return }
    ...
    try {
      ...
      if (res.status === 429) { alert('คุณส่งเรื่องบ่อยเกินไป กรุณาลองใหม่ภายหลัง'); return }
      if (!res.ok) throw new Error('ส่งเรื่องไม่สำเร็จ')
  ...
  if (result) {
    ...
    return (
      <div className="bg-card-bg border border-gray-200 dark:border-disc-border rounded-xl p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-disc-text mb-2">ส่งเรื่องเรียบร้อยแล้ว</h2>
        <p className="text-base text-gray-500 dark:text-disc-muted mb-4">
          เราส่งลิงก์ติดตามเรื่องไปทาง SMS แล้ว · กรุณาบันทึกรหัสนี้ไว้
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card-bg border border-gray-200 dark:border-disc-border rounded-xl p-6 space-y-5">
      ...
      <div>
        <label className={labelCls}>หัวข้อเรื่อง *</label>
        <input type="text" value={title} onChange={e => setTitle(e.target.value)}
          placeholder="เช่น ไฟทางสาธารณะดับหน้าหมู่บ้าน" className={inputCls} required />
      </div>

      <div>
        <label className={labelCls}>ประเภทเรื่อง</label>
      ...
      <div>
        ...
        <textarea value={detail}
          ...
          placeholder="อธิบายปัญหาที่พบ สถานที่ และรายละเอียดที่เกี่ยวข้อง..." className={inputCls} rows="5"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="ชื่อผู้ร้องเรียน" className={inputCls} required />
        </div>
        <div>
      ...
      <div>
        <label className={labelCls}>LINE ID <span className="font-normal text-gray-400 dark:text-disc-muted">(ไม่บังคับ)</span></label>
      ...
      <label className="flex items-start gap-2.5 cursor-pointer">
        ...
        <span className="text-sm text-gray-600 dark:text-disc-muted">
          ข้าพเจ้ายินยอมให้จัดเก็บและใช้ข้อมูลส่วนบุคคล (ชื่อ เบอร์โทร LINE) เพื่อติดต่อกลับและดำเนินการเรื่องร้องเรียนนี้ตามนโยบายคุ้มครองข้อมูลส่วนบุคคล (PDPA)
        </span>
      </label>
