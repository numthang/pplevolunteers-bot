...
const ICONS = {
  ...
  // ⭐ kanban/ticket/newspaper — เพิ่ม 2026-09-04 แทน pending(นาฬิกา)/pen เดิมที่ไม่สื่อความหมายกับโมดูล (user เคาะ "ขอภาพสวยๆ" ใน hamburger app switcher)
...
const APPS = [
  ...
  { key: 'cases',    label: 'CASES',     href: '/cases',          icon: 'ticket',    feature: 'cases', casesAccess: true },
  { key: 'posts',    label: 'POSTS',     href: '/posts',          icon: 'newspaper', feature: 'posts' },
  { key: 'kanban',   label: 'KANBAN',    href: '/kanban',         icon: 'kanban',    feature: 'kanban' },
              ...
              {menuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 z-20 bg-white dark:bg-disc-hover border border-warm-200 dark:border-disc-border rounded-xl shadow-lg py-2 w-64 max-h-[80vh] overflow-y-auto flex flex-col gap-0.5">
                    ...
                    {(isFinanceApp || isCallingApp || isDocsApp || isCaseModule || isPostsApp) && menuLinks.map(l => {
                      if (l.href === '/docs' && isDocsApp && docProjects.length > 0) {
                        return (
                          <div key={l.href}>
                            <Link
                    ...
                    {/* กดที่ตัวเอง = เข้า hub ของฉัน · ส่วน "แก้ไขโปรไฟล์" ด้านล่างเข้าฟอร์มตรงๆ */}
                    <a
                      href="/profile"
                      onClick={() => setMenuOpen(false)}
