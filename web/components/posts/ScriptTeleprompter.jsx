...
export default function ScriptTeleprompter({ id }) {
  ...
  // (fetch ไม่มี upload progress ที่รองรับข้ามเบราว์เซอร์จริง) มีคลิปเก่าติดโพสต์อยู่ต้องลบก่อน
  // เพราะระบบเดิมจำกัดแนบคลิปได้โพสต์ละ 1 ชิ้น (MAX_VIDEO_PER_EPISODE)
  const sendClip = useCallback(() => {
    ...
    ;(async () => {
      try {
        if (existingVideo) {
          await fetch(`/api/posts/media/${existingVideo.id}`, { method: 'DELETE' }).catch(() => {})
        }
        await new Promise((resolve, reject) => {
          const xhr = new XMLHttpRequest()
          xhr.open('POST', `/api/posts/${id}/media/video`)
