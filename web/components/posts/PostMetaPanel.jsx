...
import { Loader2, Users, UserPlus, Send, ThumbsUp, Undo2, Pencil, X } from 'lucide-react'
...
const STATUS_KEY = { draft: 'statusDraft', review: 'statusReview', approved: 'statusApproved' }
...
export default function PostMetaPanel({ id }) {
  ...
  const [mine, setMine] = useState(false)          // ฉันอยู่ในลิสต์ผู้รับผิดชอบหรือยัง — ปุ่มล่างสลับด้วยค่านี้
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) { setLoadError(data.error || t('loadFailed')); setLoading(false); return }
  ...
  async function handleRenameCategory() {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}))
      if (!res.ok) { setRenameError(json.error || t('renameFailed')); return }
      setRenameOpen(false)
  ...
  /**
   * "รับงานนี้ / ถอนตัว" — ทางเดียวที่คนลงมือเองได้โดยไม่ต้องรอใครมอบหมาย (ทรงเดียวกับหน้าเคส
   * CaseManageActions.jsx §รับเรื่อง) · ยิงแบบไม่มี body = ทำกับตัวเอง (ดู api/posts/[id]/assign)
   * ⭐ ร่างส่วนตัวก็กดได้ตั้งแต่ 2026-09-04 — เจ้าของรับงานตัวเองได้ก่อนเปิดให้ทีมเห็น
   */
  async function toggleClaim() {
    const failKey = mine ? 'leaveFailed' : 'claimFailed'
    setClaimBusy(true)
    setAssignError('')
    try {
      const res = await fetch(`/api/posts/${id}/assign`, { method: mine ? 'DELETE' : 'POST' })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) { setAssignError(data.error || t(failKey)); return }
      load()   // can.* เปลี่ยนตามความเป็นผู้รับผิดชอบด้วย (เช่น can.edit) → โหลดใหม่ให้ตรง
    } catch {
      setAssignError(t(failKey))
    } finally {
      setClaimBusy(false)
    }
  }
  ...
  async function handlePromote() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) { setPromoteError(data.error || t('promoteFailed')); return }
      setPost(data.data.post)
      load() // can.promote คำนวณจาก visibility ใหม่ — โหลดใหม่ให้ตรง
    } catch {
      setPromoteError(t('promoteFailed'))
    } finally {
      setPromoteLoading(false)
    }
  }

  if (loading) return <p className="text-warm-500 dark:text-disc-muted text-sm">{t('loading')}</p>
  ...
  const statusActions = [
    post.status === 'draft'    && can.edit           && { to: 'review',   label: t('sendReview'),      icon: <Send size={14} />,     className: 'bg-orange text-white hover:opacity-90' },
    post.status === 'review'   && can.approve        && { to: 'approved', label: t('approve'),         icon: <ThumbsUp size={14} />, className: 'bg-teal text-white hover:opacity-90' },
    // ส่งตรวจแล้วถอนกลับมาแก้เองได้ — API อนุญาตผ่าน canWritePost อยู่แล้ว
    post.status === 'review'   && can.edit           && { to: 'draft',    label: t('withdrawToDraft'), icon: <Undo2 size={14} />,    className: OUTLINE },
    post.status === 'approved' && can.requestChanges && { to: 'draft',    label: t('backToDraft'),     icon: <Undo2 size={14} />,    className: OUTLINE },
  ].filter(Boolean)
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5 text-sm">
        ...
        <div className="flex justify-between items-center gap-2">
          <span className="text-warm-500 dark:text-disc-muted shrink-0">{t('categoryLabel')}</span>
          {can.edit ? (
            ...
            <span className="text-warm-900 dark:text-disc-text truncate">{post.category || t('categoryNone')}</span>
          )}
        </div>
        {/* ห้องต้นทาง — อ่านอย่างเดียว ระบบตั้งให้ ไม่ใช่ของที่คนแก้ (ต่างจากหมวดข้างบน) */}
        {post.channel_name && (
          <div className="flex justify-between gap-2">
            <span className="text-warm-500 dark:text-disc-muted shrink-0">{t('channelLabel')}</span>
            {post.guild_id && post.channel_id ? (
              <a
                href={`https://discord.com/channels/${post.guild_id}/${post.channel_id}`}
                target="_blank"
                rel="noopener noreferrer"
                title={t('channelOpenTitle')}
        ...
        <div className="flex justify-between">
          <span className="text-warm-500 dark:text-disc-muted">{t('statusLabel')}</span>
          <span className="text-warm-900 dark:text-disc-text">{STATUS_KEY[post.status] ? t(STATUS_KEY[post.status]) : post.status}</span>
        </div>
        ...
        <div className="flex justify-between">
          <span className="text-warm-500 dark:text-disc-muted">{t('visibilityLabel')}</span>
          <span className="text-warm-900 dark:text-disc-text">{post.visibility === 'org' ? t('visibilityOrg') : t('visibilityPersonal')}</span>
        </div>

        {/* ⭐ ผู้รับผิดชอบ — **ล่างสุดของบล็อก และเป็นสองบรรทัด** (user เจอ layout แตกเอง 2026-09-04)
            ⛔ ห้ามเอากลับไปเป็น `flex justify-between` คู่ label|value: ชื่อคนไทยยาว + หลายคน
               ดันคอลัมน์ขวาแตกทันที · อยู่ล่างสุดเพื่อให้รายชื่อยาวลงข้างล่างได้เรื่อยๆ
               โดยไม่เบียดแถวอื่น (หมวด/ห้อง/สถานะ ความสูงคงที่ ต้องอยู่บน)
            ⭐ ร่างส่วนตัว **มีแถวนี้แล้ว** ตั้งแต่ 2026-09-04 — เจ้าของกดรับงานตัวเองได้
               แต่ยังไม่มี combobox (เพิ่มคนอื่นไม่ได้ ดู can.assign ที่ api/posts/[id]/route.js) */}
        {(can.assign || can.claim || assignees.length > 0) && (
          <div className="flex flex-col gap-1 pt-1">
            <span className="text-warm-500 dark:text-disc-muted">{t('assigneesLabel')}</span>
            {can.assign ? (
              <TagCombobox
                type="multi_select"
                numericIds={false}
              ...
              <span className={`break-words ${assignees.length === 0 ? 'text-warm-500 dark:text-disc-muted' : 'text-warm-900 dark:text-disc-text'}`}>
      ...
      {/* แถวปุ่มของการ์ดนี้ — รวมไว้ที่เดียวใต้เส้นคั่น ห้ามแทรกกลางแถว label/value ข้างบน (ดูรก)
          ...
          เหมือนที่ทำกับ "หมวด" */}
      {(statusActions.length > 0 || can.promote || can.claim) && (
        <div className="pt-3 border-t border-warm-200 dark:border-disc-border flex flex-col gap-2">
          <div className="flex flex-wrap gap-2">
            ...
            {/* รับงาน/ถอนตัว — ป้ายบอกบทที่จะได้ เหมือนการ์ด kanban: ยังไม่มีใครรับ = "รับงานนี้"
                มีคนรับแล้ว = "ลงมือด้วย" · ตัวเองรับอยู่ = "ถอนตัว" */}
            {can.claim && (
              <button
                onClick={toggleClaim}
                disabled={claimBusy}
                className={`flex-1 min-w-[45%] flex items-center justify-center gap-1.5 px-3 py-2 text-sm rounded-lg disabled:opacity-40 transition ${
                  mine ? OUTLINE : 'bg-orange text-white hover:opacity-90'
                }`}
              >
                {claimBusy ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} />}
                {mine ? t('leave') : assignees.length ? t('join') : t('claim')}
              </button>
            )}
            {can.promote && (
              <button
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <h2 className="text-lg font-semibold text-warm-900 dark:text-disc-text">{t('renameTitle', { category: post.category })}</h2>
              <button
                onClick={() => setRenameOpen(false)}
            ...
            <div>
              <label className="block text-sm font-medium text-warm-700 dark:text-disc-muted mb-1">{t('renameNewLabel')}</label>
              <input
                autoFocus
                value={renameTo}
