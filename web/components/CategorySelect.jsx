...
export default function CategorySelect({ categories = [], value, onChange, placeholder = 'ไม่ระบุ', className = '' }) {
  ...
  const dropdown = open && rect && createPortal(
    <div ref={dropRef}
      ...
      className="bg-white dark:bg-disc-hover border dark:border-disc-border rounded-xl shadow-lg max-h-56 overflow-y-auto"
    >
      ...
      {categories.map(c => (
        <button key={c.id} type="button" onClick={() => { onChange(String(c.id)); setOpen(false) }}
          className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/40
            ${String(c.id) === String(value) ? 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400' : 'text-gray-900 dark:text-disc-text'}`}>
          <CatIcon name={c.icon} size={15} className="flex-shrink-0 text-gray-500 dark:text-disc-muted" />
          <span>{c.name}</span>
        </button>
      ))}
    </div>,
  ...
  return (
    <div className={className}>
      ...
      >
        {selected ? (
          <>
            <CatIcon name={selected.icon} size={15} className="text-gray-500 dark:text-disc-muted flex-shrink-0" />
