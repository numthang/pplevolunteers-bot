...
lines.push('  (org_id, first_name, last_name, phone, category, province, amphoe, note)');
lines.push('VALUES');
lines.push(contacts.map((c, idx) => {
  const comma = idx < contacts.length - 1 ? ',' : '';
  return `  ((SELECT org_id FROM dc_guilds WHERE guild_id=${esc(GUILD_ID)}), ${esc(c.firstName)}, ${esc(c.lastName)}, ${esc(c.phone)}, 'donor', ${esc(c.province)}, ${esc(c.amphoe)}, ${esc(c.note)})${comma}`;
}).join('\n'));
lines.push(';');
lines.push('');
lines.push('-- Done.');
