...
==
• ทีมบรรณาธิการ => สื่อ 
• อาสาสู้ภัยพิบัติ => อาสาส้มสู้ไฟ อาสาส้มสู้น้ำ
• ผู้สมัครเขตภาคกลาง => ผู้สมัคร
• ทีมพื้นที่/ร้องเรียน => เรื่องร้องเรียน กระทู้เรื่องร้องเรียน
• ทีมตัวแทนสมาชิก => ตัวแทนสมาชิก
• ทีมData
• ทีมระดมทุน
• ผู้สมัครส.ก.
• ฝ่ายเครือข่าย
...
• สส.พรรค
== 
สรุปขั้นตอนทั้งหมดครับ:

1. ติดตั้ง Meilisearch


curl -L https://install.meilisearch.com | sh
sudo mv meilisearch /usr/local/bin/
2. ตั้ง systemd service
...
[Service]
ExecStart=/usr/local/bin/meilisearch --master-key=YOUR_KEY --db-path /var/lib/meilisearch
Restart=always
User=root
...
mysql -u pple_dcbot -p pple_volunteers < scripts/migration-forum.sql
5. ติดตั้ง npm package
...
pm2 restart bot  # หรือ process ที่ใช้อยู่
8. Backfill


node scripts/backfill-forum.js --guild GUILD_ID
9. Setup dashboard (ทำใน Discord)

/panel forum channel:#ช่องที่ต้องการ ทีละช่องที่อยากมี dashboard
ข้อ 1-3 ทำแล้วในเครื่อง dev ใช่ไหมครับ? หรือ VPS production เป็นคนละเครื่องกัน?

ตอนนี้ทำหมดแล้วบนเครื่อง dev เครื่องตัวเอง vps คือ production
โอเคครับ งั้น deploy ขึ้น VPS:

1. push code ขึ้น git


git add .
git commit -m "forum search system"
git push
2. บน VPS — pull + ติดตั้ง


git pull
npm install
3. ติดตั้ง Meilisearch บน VPS


curl -L https://install.meilisearch.com | sh
sudo mv meilisearch /usr/local/bin/
ตั้ง systemd + เพิ่ม .env เหมือนเดิม (key ใหม่สำหรับ production ได้เลย)

4. รัน SQL บน VPS


mysql -u pple_dcbot -p pple_volunteers < scripts/migration-forum.sql
5. Deploy commands + restart bot


node deploy-commands.js
pm2 restart bot  # หรืออะไรที่ใช้รัน bot บน VPS
6. Backfill